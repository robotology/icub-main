		 ...
		 double& la, double& lb)  
{
  ...
  double pvec[7];
  fitter.Apply(x_in,y_in,num_points,
	       pvec,
	       xc,yc,xa,ya,la,lb);
  double eps = 1.0e-5;
  if (fabs(xa)>fabs(ya))
...
int FitEllipse::generate() {
  ...
  bool good = true;
  for (int i=0; i<4; i++) {
    if (xx[i]<0||yy[i]<0||xx[i]>1000||yy[i]>1000) {
      good = false;
    }
  }
  ...
  if (good) {
    /*
      double chk =
	Axx*xx1*xx1 + Axy*xx1*yy1 + Ayy*yy1*yy1 + Ax*xx1 + Ay*yy1 + Ao;
      double chk2 =
	Axx*xx2*xx2 + Axy*xx2*yy2 + Ayy*yy2*yy2 + Ax*xx2 + Ay*yy2 + Ao;
      double chk3 =
	Axx + Axy + Ayy + Ax + Ay + Ao;
    */
      int ok = 1;
      
      for (int i=0; i<4; i++) {
	assignf(1e8,Axx,Axy,Ayy,Ax,Ay,Ao);
	ok = ok && draw(xx[i],yy[i],xx[(i+1)%4],yy[(i+1)%4]);
      }
      //printf("Switch...\n");
      //assignf(1e8,Axx,Axy,Ayy,Ax,Ay,Ao);
      //int ok2 = draw(xx2,yy2,xx1,yy1);
      int ok2 = 0;
      if (0&&(1||ok>=0||ok2>=0)) {

	printf("Got an ellipse\n");
	/*
	printf("%gxx %gxy %gyy %gx %gy %g // %g %g // %g %g // %g %g %g\n",
	       Axx, Axy, Ayy, Ax, Ay, Ao,
	       xx1, yy1,
	       xx2, yy2,
	       chk, chk2, chk3);
  ...
  return len;
  //xs.resize(num_points,1);
  //ys.resize(num_points,1);
  //EllipseFitter::get_param(pvec,num_points,&(xs(0,0)),&(ys(0,0)));
}
