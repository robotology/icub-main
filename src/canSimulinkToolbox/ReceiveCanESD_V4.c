...
#define S_FUNCTION_NAME ReceiveCanESD_V4
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  //  DECL_AND_INIT_DIMSINFO(outputDimsInfo);
    ...
    ssSetNumOutputPorts(S, (long) mxGetN(CAN_ID_ARG));
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    ret =canTake(m_h0, rx_message, &len);
    if(ret == NTCAN_SUCCESS)
    {
        for (i=0;i<len;i++)
        {
            ...
            {
                for(j=0;j<(int) mxGetN(CAN_ID_ARG);j++)
                {
                    if (rx_message[i].id==(int) mxGetPr(CAN_ID_ARG)[j])
                    {
                        for (d=0;d<8;d++)
                        {
                            p_data[j][d]=(int_T) rx_message[i].data[d];
                           // y[j][d]=(int_T) rx_message[i].data[d]; // j is the index of the output port
                          
                           // memcpy(y[j],p_data,8);
                        }
                         //value=(double *) &p_data[j][0];
                          y[j][0]=*(double *) &p_data[j][0];
                    }
