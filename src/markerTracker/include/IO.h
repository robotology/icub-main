...
public:
	...
	~Connection();
	bool connect(const char *_input_port, const char *_output_port, const char *_type="tcp");
...
//	ImgInputFile(Parameters *_params_ptr, string& _portname, int _channel);		//Mattia
	~ImgInputFile();
...
//	ImgInputRobot(Parameters *_params_ptr, string& _portname);	//Mattia
	~ImgInputRobot();
	ImageOf<PixelRgb>* readImg();	
};
...
//	ImgOutputRobot(string& _portName);	//Mattia
	~ImgOutputRobot();
