...
int main(int argc, char *argv[]) {
	...
	// Display
	ImgOutputRobot	img_result_out_ptr (p.cam1_out_port.c_str());
	ImgOutputRobot	img_alt_result_out_ptr (p.cam2_out_port.c_str());

	// output ports for the tracker positions	
	DataOutputRobot posOutputLeft_port (p.dataLeft_port.c_str());
	DataOutputRobot posOutputRight_port (p.dataRight_port.c_str());



	if (p.execution == "robot") {
		// robot mode
		imageInput1_ptr	= new ImgInputRobot	(&p, p.cam1_in_port.c_str());
		if (p.stereovision) {
			imageInput2_ptr	= new ImgInputRobot	(&p, p.cam2_in_port.c_str());
		}
		...
		// file mode
		imageInput1_ptr	= new ImgInputFile (&p, p.cam1_in_port.c_str(), 1);
		if (p.stereovision) {
			imageInput2_ptr	= new ImgInputFile (&p, p.cam2_in_port.c_str(), 2);
		}
	}

	ImgInput		*img_ins_ptr [2] = { imageInput1_ptr, imageInput2_ptr };
	ImgOutput		*img_outs_ptr [2] = { &img_result_out_ptr, &img_alt_result_out_ptr };
	DataOutput		*data_out_ptr [2] = { &posOutputLeft_port, &posOutputRight_port};
	...
	conn.connect(posOutputLeft_port.getPortname(), p.trackerLeft_out_port.c_str(), "udp");
	conn.connect(posOutputRight_port.getPortname(), p.trackerRight_out_port.c_str());
	...
	while (loop) {		
		...
		if (param == "quit") {
			...
			LaunchApplication app (img_outs_ptr, img_ins_ptr, &p, data_out_ptr);
			app.launch();
	...
	conn.disconnect(posOutputLeft_port.getPortname(), p.trackerLeft_out_port.c_str());
	conn.disconnect(posOutputRight_port.getPortname(), p.trackerRight_out_port.c_str());
