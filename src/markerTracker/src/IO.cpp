...
bool Connection::connect(const char *_inputPort, const char *_outputPort, const char *_type) {
	//printf("Connecting:\n\tport %s with \n\tport %s\n", _inputPort, _outputPort);
  	bool b;
	b = Network::connect(_inputPort, _outputPort, _type);
	if (b) {
		printf("Info:\t[IO.Connection]\t{[%s] input port connected to}\n",_inputPort);
		printf("     \t               \t{[%s] output port in %s-mode}\n",_outputPort, _type);
	}
	else {
		printf("Info:\t[IO.Connection]\t{[%s] output port NOT connected to}\n",_outputPort);
		printf("     \t               \t{[%s] input port}\n",_inputPort);
	}
//	printf("\n finishing to connect\n");
	return b;
}
...
ImgInputRobot::ImgInputRobot(Parameters *_params_ptr, const char *_portname) : ImgInput(_params_ptr, _portname) {
	...
	if (_params_ptr->rec_perception) {
		this->imgOut_ptr = new ImgOutputFile ("recording");
		this->cnt = 0;
	}
}
...
ImageOf<PixelRgb>* ImgInputRobot::readImg() {
	...
	if (this->params_ptr->rec_perception) {
		char tmp1 [16];
		sprintf(tmp1, "%s%d", this->getPortname(),  this->cnt);
...
DataOutput::DataOutput(const char *_portName_ptr, bool _createPort) : Output(_portName_ptr) {
	...
	//printf("Info:\t[IO.Connection]\t{%s output port created}\n", _portName_ptr);
}
...
// Constructor
DataOutputFile::DataOutputFile(const char *_fileName_ptr) : DataOutput(_fileName_ptr, false) {
}
...
// Constructor
DataOutputRobot::DataOutputRobot(const char *_portName_ptr) : DataOutput(_portName_ptr, true) {
	//printf("created a port with name : [%s] \n", _portName_ptr);
}
// Destructor
