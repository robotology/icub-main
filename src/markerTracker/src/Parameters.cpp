...
void Parameters::printAndAsk(const char *_param_ptr, bool *_value_ptr) {
	...
	sprintf(tmp, "Current %s value is now: %s. Enter your new choice:", _param_ptr, value.c_str());
	cout << tmp << endl;
	cin >> value;
	//tmp = "";
...
void Parameters::loadParameters() {
	...
	this->trackerLeft_out_port		= this->getTextParameter("OUTPORTS", "tracker_pos_left_out_port");
	this->trackerRight_out_port		= this->getTextParameter("OUTPORTS", "tracker_pos_right_out_port");
	...
	this->dataLeft_port		= "/markerpos/left";
	this->dataRight_port	= "/markerpos/right";
