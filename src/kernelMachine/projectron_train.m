function model = projectron_train(X,Y,model)
% PROJECTRON_TRAIN Projectron
%    model = projectron_train(X,Y,model)
...
for i=1:n
    ...
    if Y(i)*val_f<=0
        ...
        if size(model.SV,2)>0
            ...
            delta=Kii-coeff*K_f';
            norm_xt=Kii-delta;
            delta_max=((2*loss-norm_xt-0.5)/(2*model.U))^2;
        end
    ...
    if mod(i,1000)==0
        fprintf('#%d SV:%g(%d)\tpred:%g\tskip:%g\tproj1:%g\tAER:%g\n', ...
            ceil(i/1000),numel(model.S)/i*100,numel(model.S),n_pred/i*100,n_skip/i*100,n_proj1/i*100,model.aer(end)*100);
        %fprintf('\n');
        %fprintf('.');
    end
