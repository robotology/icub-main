...
int import_lowe_features( char* filename, struct feature** features ) {
    ...
    /*allocating feature array*/
    f = (feature*) calloc( n, sizeof(struct feature) );
    for( i = 0; i < n; i++ ) {
        /*allocating space for detection data*/
        ddata = (detection_data*) malloc( sizeof( struct detection_data ) );
        memset( ddata, 0, sizeof( struct detection_data ) );
        /* read detection parameters */
        if( fscanf( file, " %d %d %d %d ", &oct, &inter, &row, &col ) != 4 ) {
            fprintf( stderr, "Warning: error reading detection parameters of feature #%d, %s, line %d\n",
                     i+1, __FILE__, __LINE__ );
            free( f );
            return -1;
        }
        ...
        ddata->r = row;
        ddata->c = col;
        f[i].feature_data = ddata;

        /* read affine region parameters */
        if( fscanf( file, " %lf %lf %lf %lf ", &y, &x, &s, &o ) != 4 ) {
            fprintf( stderr, "Warning: error reading affine parameters of feature #%d, %s, line %d\n",
                     i+1, __FILE__, __LINE__ );
            free( f );
            return -1;
        }
...
/*
  ...
  @return Returns the number of features imported from filename or -1 on error
*/
int import_lowe_features_no_detection_data( char* filename, struct feature** features ) {
    struct feature* f;
    int i, j, n, d;
    double x, y, s, o, dv;
    FILE* file;
...
/*
  ...
  @return Returns 0 on success or 1 on error
*/
int export_lowe_features_no_detection_data( char* filename, struct feature* feat, int n ) {
    FILE* file;
    int i, j, d;
...
int export_lowe_features( char* filename, struct feature* feat, int n ) {
    ...
    fprintf( file, "%d %d\n", n, d );
    for( i = 0; i < n; i++ ) {
        if(feat[i].feature_data != NULL) {
            ddata = feat_detection_data( (feat+i) );
            fprintf( file, "%d %d %d %d\n", ddata->octv, ddata->intvl, ddata->r, ddata->c );
        } else
            fprintf( stderr, "Warning: feature_data of feature %d was NULL\n",i);
