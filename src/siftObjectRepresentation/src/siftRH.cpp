 ...
 *          2006-2007  Rob Hess <hess@eecs.oregonstate.edu>
...
void clone_feature(struct feature* new_feat , struct feature* feat);
...
int _sift_features( IplImage* img, struct feature** feat, int intvls,
    ...
    // printf("%d, %f, %f, %d, %d, %d, %d\n", intvls, sigma, contr_thr, curv_thr, img_dbl, descr_width, descr_hist_bins); //DEBUG
    // disp_im(img); //DEBUG
    ...
    init_img = create_init_img( img, img_dbl, sigma );         		//disp_im(init_img); //DEBUG
    ...
    features = scale_space_extrema( dog_pyr, octvs, intvls, contr_thr,
                                    curv_thr, storage );		//printf("total features1: %d\n",features->total);//DEBUG
    ...
    n = features->total;							//printf("total features2: %d\n",n);//DEBUG
    *feat = (feature *) calloc( n, sizeof(struct feature) );
    *feat = (feature *) cvCvtSeqToArray( features, *feat, CV_WHOLE_SEQ );
    //   for( i = 0; i < n; i++ )	//RB frees this data, but we need it to turn features into keypoints
    //     {
...
IplImage* create_init_img( IplImage* img, int img_dbl, double sigma ) {
    ...
    gray = convert_to_gray32( img );				//disp_im(img); //DEBUG
    if( img_dbl ) {
...
void calc_feature_oris( CvSeq* features, IplImage*** gauss_pyr ) {
    ...
    double* hist;
    double omax;
    int i, j, n = features->total; 			//printf("calc_feature_oris : total features: %d\n",n);//DEBUG
...
/*
  ...
  @param feat feature to be cloned
  @param new_feat pointer to empty feature space
*/
void clone_feature(struct feature* new_feat , struct feature* feat) {
    //struct feature* new_feat;
    struct detection_data* ddata;
