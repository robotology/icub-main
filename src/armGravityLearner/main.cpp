...
const double START_POSITION[]={-30, 0, 0, -10, 0, 0, 0, 0, 0, 0, 0, 0, 60, -60, 0};
...
//const double MAX_POSITION[]={   -30, 40, 90,  -45, 0, 0, 0, 0, 0, 0, 0, 0, 60, -60, 0};
//const double MIN_POSITION[]={ -80, -40, 0, -45, 0, 0, 0, 0, 0, 0, 0, 0, 60, -60, 0};

const double MAX_POSITION[]={ -40,  80, 120, -45,  0,  0,  0};
const double MIN_POSITION[]={ -120,-50, -30, -45,  0,  0,  0};
...
class GravityTest: public RateThread
{
private:
    IEncoders *iencs;
    IAmplifierControl *iamps;
	...
	int count;
public:
    GravityTest(PolyDriver *Arm_d, int rate): RateThread(rate),
        currents(0)
    { 
        Arm_dd=Arm_d;
        Arm_dd->view(iencs);
	...
	bool threadInit()
	{
		...
		gravityModel[0].ShowInfo();

		errorFile=fopen("error.log", "w");
		dataFile=fopen("data.txt", "w");
		count=0; 

		ipos->setRefSpeed(3, ARM_VELOCITY[3]);
		ipos->positionMove(3, MAX_POSITION[3]);

		ipids->getPid(0, &pids[0]);
		ipids->getPid(1, &pids[1]);
		ipids->getPid(2, &pids[2]);
		...
		pids[0].setOffset(0.0);
		pids[1].setOffset(0.0);
		pids[2].setOffset(0.0);

		ipids->setPid(0, pids[0]);
		ipids->setPid(1, pids[1]);
		ipids->setPid(2, pids[2]);
    ...
    void run()
    {
		...
		voltages[0]=gravityModel[0].Predict(angularPositions,3);
		voltages[1]=gravityModel[1].Predict(angularPositions,3);
		voltages[2]=gravityModel[2].Predict(angularPositions,3);

		//gravityModel[0].ShowDetails();
		count++; 

		double gain=1333.0/12.0;
		pids[0].setOffset(gain*voltages[0]);
		pids[1].setOffset(gain*voltages[1]);
		pids[2].setOffset(gain*voltages[2]);
...
private:
	...
	int count;
public:
    GravityCompensator(PolyDriver *Arm_d, int rate): RateThread(rate),
        currents(0)
    { 
        Arm_dd=Arm_d;
        Arm_dd->view(iencs);
...
//#if 0
	GravityCompensator *gc=new GravityCompensator(&armdd, 40);
	gc->start();
    char cmd[80];
    bool quit=false;
    while (!quit) 
