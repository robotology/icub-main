 ...
 *
 * \f[
 * \mathbf{q}=\arg\min_{\mathbf{q}\in R^{n} }\left(\frac{1}{2}\left\|\mathbf{\alpha}_d-\mathit{K_{\alpha}}\left(\mathbf{q}\right)\right\|^2+\mathit{w}\cdot\frac{1}{2}\left(\mathbf{q}_{\text{rest}}-\mathbf{q}\right)^{\top}\mathit{W}_{\text{rest}}\left(\mathbf{q}_{\text{rest}}-\mathbf{q}\right)\right) \quad \text{s.t.}\,\left\{\begin{array}{l}\left\|\mathbf{x}_d-\mathit{K_x}\left(\mathbf{q}\right)\right\|^2<\epsilon\\\mathbf{q}_L<\mathbf{q}<\mathbf{q}_U\end{array}\right.
 * \f]
 *
 ...
 * \f$ \mathbf{q}_{\text{rest}} \f$ is used to keep the solution as close as possible to a given rest
 * position in the joint space (weighting with a positive factor \f$ \mathit{w} < 1 \f$ and also
 * through the diagonal matrix \f$ \mathit{W}_{\text{rest}} \f$ which allows to select a specific weight for each joint)
