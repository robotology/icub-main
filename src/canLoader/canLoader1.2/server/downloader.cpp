...
//*****************************************************************/
int cDownloader::change_board_info(int target_id, const char* board_info)
{
 // check if driver is running
 if (m_candriver == NULL)
 {
...
int getvalue(const char* line, int len)
{
...
int cDownloader::download_motorola_line(const char* line, int len, int board_pid)
{
...
int cDownloader::download_file(int board_pid)
{
 ...
 if (file_data.size()==0)
  ...
  if (file_data.size()!=0)
  {
	  yarp::String s = file_data.front();
	  file_data.pop_front();

	  //avoid to download empty lines
	  if (s.length()!=0)
		{
		  ret = download_motorola_line(s.c_str(), s.length(), board_pid);		  
		  if (ret != 0)
		  {
