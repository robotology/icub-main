...
{  
 ...
 printf  ("opened firmware version: %s %d", substring, firmware_version);
 

 strncpy (substring, buffer+6, 2);
 firmware_revision = strtol(substring,  NULL, 10);
 printf  ("opened firmware revision: %s", substring, firmware_revision);
 ...
 for (i=0; i<downloader.board_list_size; i++)
 {
	 if (downloader.board_list[i].status==BOARD_RUNNING &&
		 downloader.board_list[i].selected==true)
		 {	
			if (downloader.startscheda(downloader.board_list[i].pid)!=0)
				{
 ...
 do
	{
	 ret = downloader.download_file(0x0F);
	 if (ret==1)
		 {
...
edited_board_id (GtkCellRendererText *cell, gchar *path_str, gchar *new_text, gpointer data)
{
  ...
  {
	  downloader.change_card_address(*old_val,new_val);
	  delete (old_val);
  }
...
*/
    ...
    gtk_window_set_title (GTK_WINDOW (window), "CAN Flasher V1.2 ");
