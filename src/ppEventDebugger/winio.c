...
void outp(unsigned short address, unsigned char byte);
unsigned char inp(unsigned short address);
...
void outp(unsigned short address, unsigned char byte)
{
    ...
    *pBuffer = address;
    ...
    if (!error) 
	printf("Error occured in outp %d\n",GetLastError());
}

unsigned char inp(unsigned short PortAddress)
{
    unsigned int error;
    DWORD BytesReturned;
    unsigned char Buffer[3];
