...
const double START_POSITION_ARM[]={-30, 50, 0,  -10, 0, 0, 0, 0, 0, 0, 0, 0, 60, -60, 0};
...
public:
	...
	bool threadInit()
	{
		...
		ballPort=new CollectorPortHandler;
		ballPort->open("/james/armfwd/ball/i");
		ballPort->start();
		return true;
	}
	...
	{		
		...
		double minError=2e10; //big number
		while(!done)
		{
			...
			// now check limits
			bool valid = checkLimitsArm(candidatePosArm);
			
			if (valid)
			{
				if (err<1)
				{
					memcpy(bestTrial, candidatePosArm, sizeof(double)*ARM_JOINTS);
					minError=err;
					done=true;
				}
		...
		if(foundOne)
		{
			fprintf(stderr, "Best error is: %.1lf trials: %d", minError, count);
			fixedTimeMove(encodersArm, bestTrial, encodersHead, candidatePosHead, MOVE_TIME);
		}
		else
			fprintf(stderr, "Did not find any good position, aborting after %d trials\n", count);
	}
...
int main(int argc, char *argv[]) 
{
    ...
    fprintf(stderr, "Opening the net...");
...
//	readPositions(argv[1], exploredPositions);
    ...
    armOptions.put("local", "/james/armfwd/client");   //local port names
	...
	fixation.open("/james/armfwd/fixation");
	...
	Terminee terminee("/james/armfwd/quit");
    if (!terminee.isOk()) { 
        fprintf(stderr, "armFwd --> Failed to create proper quit socket\n"); 
       return 1;
    }   

    fprintf(stderr, "armFwd --> Driver instantiated and running (silently)\n");
    while (!terminee.mustQuit()) 
	{
		Time::delay(1); 
    }
		
	fprintf(stderr, "armFwd --> Received a quit message\n");
