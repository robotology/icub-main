...
int main(int argc, char * argv[])
{
  ...
  double Threshold2=1.0;//sends beats if human stays silent from 1 sec.
  ...
  Threshold2 = (config.findGroup("GENERAL").find("game_time").asDouble());
  
  printf("TH %lf ",Threshold2 );//DELETE
  ...
  if (grabber!=NULL)
    {
	  ...
	  ss=st1;
      //ftime2=ftime;
      while (1)//while(difftime(ftime, stime)<total_time)
	{
	  ftime1=ftime;
	  while (stop==0)//while((stop==0)&&(difftime(ftime, stime)<total_time))
	    {
	      if (grabber->getSound(sound))
		{
		  for (i=0;i<sound.getSamples();i++)
		      ...
		      for(j = 0; j < sound.getChannels(); j++)
			{
			  ...
			  //frank: what is the significance of this value?
	      ...
	      else
		{
	      ...
	      time(&ftime);
	    //  duration=difftime(ftime, ftime2);
		 
		//  duration_double=difftime(ftime, ftime2);
	      //if (duration>Threshold2)
	//	  d1= duration_double * 1000;
	//	  d2 = Threshold2 * 1000;
		  fs=clock();
