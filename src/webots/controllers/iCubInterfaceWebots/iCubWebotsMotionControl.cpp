...
const double maxPosition = WB_SERVO_INFINITY;
const double minPosition = -WB_SERVO_INFINITY;
...
bool iCubWebotsMotionControl::open(yarp::os::Searchable& config) {
    ...
    joint_dev = new WbDeviceTag[njoints];
    servo_minPos = new WbFieldRef[njoints];
    servo_maxPos = new WbFieldRef[njoints];
    
    minPos = new double[njoints];
    maxPos = new double[njoints];
    ...
    for(int i =0;i<njoints;i++)
        {
            joint_dev[i] = wb_robot_get_device(jointNames[i].c_str());
            nodes[i] = wb_supervisor_node_get_from_def(jointNames[i].c_str());
            if(joint_dev[i]==0)
                {
                ...
                wb_servo_enable_position(joint_dev[i],positionUpdateRate);
...
{
	ACE_OS::printf("Closing the controller... \n");
    if (_opened) {
    ...
    while(!isStopping())
        {
            ...
            //ACE_OS::printf("vel for joint 0 %f\n", next_sp[0]); 
...
bool iCubWebotsMotionControl::setPidRaw (int axis, const Pid &pid)
{
    if(axis<njoints)
        {
            _mutex.wait();
            wb_servo_set_control_p(joint_dev[axis],(float)pid.kp);
...
bool iCubWebotsMotionControl::setPidsRaw(const Pid *pids)
{
    ...
    for(int i=0;i<njoints;i++)
        {
            wb_servo_set_control_p(joint_dev[i],(float)pids[i].kp);
...
bool iCubWebotsMotionControl::setRefSpeedRaw(int axis, double sp)
{
    if(axis<njoints)
        {
            _mutex.wait();
            wb_servo_set_velocity(joint_dev[axis],(float)sp);
            refSpeed[axis] = sp;
            _mutex.post();
            return true;
        }
...
bool iCubWebotsMotionControl::setRefSpeedsRaw(const double *spds)
{
    ...
    for(int i =0;i<njoints;i++)
        {
            wb_servo_set_velocity(joint_dev[i],(float)spds[i]);
            refSpeed[i] = spds[i];
        }
    _mutex.post();
    return true;
...
bool iCubWebotsMotionControl::setRefAccelerationRaw(int axis, double acc)
{
    if(axis<njoints)
        {
            _mutex.wait();
            wb_servo_set_acceleration(joint_dev[axis],(float)acc);
            refAccel[axis] = acc;
            _mutex.post();
            return true;
        }
...
bool iCubWebotsMotionControl::setRefAccelerationsRaw(const double *accs)
{
    ...
    for(int i = 0;i<njoints;i++)
        {
            wb_servo_set_acceleration(joint_dev[i],(float)accs[i]);
            refAccel[i] = accs[i];
        }
    _mutex.post();
    return true;
...
bool iCubWebotsMotionControl::velocityMoveRaw (int axis, double sp)
{
    if(axis<njoints){
        ...
        if(sp > maxVelocity)
           ...
           ACE_OS::printf("receiving velocity command\n"); 
        _mutex.post();
    }
    return true;
}
     ...
     if(axis<njoints)
        {
            _mutex.wait();
            motor_on[axis] = false;
            wb_servo_set_force(joint_dev[axis],0.0);
            _mutex.post();
            return true;            
        }
