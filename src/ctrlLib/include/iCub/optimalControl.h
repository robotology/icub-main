 ...
 * where: \f[ \begin{array}{l} L_i = \left(P+B^\top T_{i+1} B\right)^{-1} B^\top T_{i+1} A \f] \n whilst \f$ T_i \\ T_N = V_N \\ T_i = V + A^\top \left[ T_{i+1} - T_{i+1} B \left(P+B^\top T_{i+1} B\right)^{-1} B^\top T_{i+1} \right] A \end{array} \f]
 *
 *
