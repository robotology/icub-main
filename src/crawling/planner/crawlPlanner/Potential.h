...
#define kp 2
#define kn 4
#define POSITIVE_GRADIENT_EXPRESSION (1/(distance/10) - 1/d0)*(1 / pow(distance,2)) // the gradient at point P is inversely proportional to distance^kp.
#define NEGATIVE_GRADIENT_EXPRESSION (1 / pow((distance/10), kn)) // the gradient at point P is inversely proportional to distance^kn.
...
public:
	...
	/**
    * Default constructor of the Potential class.
    * Does nothing
    */
	Potential(void);

	/**
    * Constructor of the Potential class.
    * Defines a potential at position x, y with radius myRadius and potential myPotential
    */
	Potential(double x, double y, double myRadius, double myPotential, int myAge = 0);

	/**
    * Copy constructor of the Potential class.
    * Creates a copy of the specified potential;
    */
