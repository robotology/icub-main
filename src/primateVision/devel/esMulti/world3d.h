...
public:
  ...
  ~world3d();
  void callback();
  void setim(QImage *qim_, QImage *depth_, double rx_, double ry_, double roll_, double pitch_, double yaw_, float a_, int w_, int h_, int left_, int top_);
...
protected:
  ...
  int find_min_grey(QImage *src, int w, int h);
  int find_max_grey(QImage *src, int w, int h);
  void set_grey_params(QImage *src, int tot_lev, int w, int h);
  void set_current_mask(QImage *src, QImage *mask, int lev, int w, int h);
  void create_disparity_map(QImage *src, QImage *dest, QImage *mask, int w, int h, int left, int top);
  void draw_multisphere(QImage *src, QImage *depth, int max_disp, int w, int h, int top, int left);
...
private:
  ...
  QImage *qim;            //original cartesian image, with transparent borders  
  QImage *log_im;         //current log representation
  QImage *depth_map;      //greyscale disparity map
  ...
  int top_bord;           //size of horizontal borders
  int left_bord;          //size of vertical borders  

  int abs_disp;           //maximum disparity value
