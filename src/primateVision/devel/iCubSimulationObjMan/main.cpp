			...
			if (subcmd=="get"||subcmd=="set"||subcmd=="mk"||subcmd=="grab"||subcmd=="rot") {
				...
				switch (id) {
					...
					case VOCAB4('b','a','l','l'):
						...
						break;
					case VOCAB3('b','o','x'):
						bid = odeinit._iCub->body[1];
						setBody = 2;
						break;
					case VOCAB4('s','b','o','x'):
						bid2 = odeinit._iCub->geom[1];
						setBody = 5;
						break;
					case VOCAB3('c','y','l'):
						bid = odeinit._iCub->body[1];
				...
				if (bid==NULL || bid2==NULL) {
					...
					if (subcmd=="set") {
                        ...
                        if (setBody==8){
							...
							if ((N >(i+1)) || (N < 1 )){reply.addString("object not known");}
                                ...
                                double rad = command.get(7).asDouble();
                               // odeinit._wrld->skata = true;
                                odeinit._wrld->active[N-1] = true;
                                odeinit._wrld->s_labl_obj[N-1].radius = rad;
                                odeinit.mutex.wait();
                                dGeomSetPosition(odeinit._wrld->s_labl_obj[N-1].labelgeom[0],x,y,z);
                                odeinit.mutex.post();
                            }                      
                        }                        
					...
					if (subcmd=="mk"){ //this allows the user to create some boxes around the world		
                        if (setBody==8){
							odeinit._wrld-> passed = false;
                            if (num < MAXNUM){	
								i = odeinit._wrld->S_lablOBJNUM;
								odeinit._wrld->S_lablOBJNUM++;
							}       
                            //odeinit._wrld->active[i] = true;
                            //cout << "Setting " << i << " " << odeinit._wrld->active[0] << endl;       
                            odeinit._wrld->s_labl_obj[i].radius = command.get(3).asDouble();//radius
							...
							odeinit._wrld->labelstring = command.get(7).asString();
                           // int lbl = command.get(7).asInt(); 
							int lbl = 11;
							char* temp = (char* ) odeinit._wrld->labelstring.c_str();
							cout << temp << endl;

							if (odeinit._wrld->labelstring == "bottle"){
								cout << "this is a bottle" << " Is thexture passed? " << odeinit._wrld-> passed << endl;
								lbl = 4;
								
								odeinit._wrld->objManText ++;
								odeinit._wrld->s_labl_obj[i].texture = odeinit._wrld->objManText;
								cout << "man texture " <<  odeinit._wrld->objManText << " the increment is " << i << " texture is " << odeinit._wrld->s_labl_obj[i].texture << endl;
							}

							if (odeinit._wrld->labelstring == "fags"){
								cout << "this is a fags" << endl;
								lbl = 5;
							}
                            if (odeinit._wrld->labelstring == "coke"){
								cout << "this is a coke" << endl;
								lbl = 2;
							}
							if (odeinit._wrld->labelstring == "unknown"){
								cout << "this is a unknown" << endl;
								lbl = 6;
							}
                            odeinit.mutex.wait();
                            odeinit._wrld->s_labl_obj[i].labelgeom[0] = dCreateSphere (odeinit._iCub->iCub,odeinit._wrld->s_labl_obj[i].radius);
							dGeomSetPosition(odeinit._wrld->s_labl_obj[i].labelgeom[0],x,y,z);            
                            odeinit._wrld->s_labl_obj[i].label = lbl;
                            odeinit.mutex.post();
                        }
						if (setBody==2){
						...
						if (setBody==7){
							...
							odeinit._wrld->s_cyl_obj[i].cylgeom[0] = dCreateCylinder (odeinit.space,odeinit._wrld->s_cyl_obj[i].radius,odeinit._wrld->s_cyl_obj[i].lenght);       odeinit.mutex.wait();   
...
//a better flip, but simply defining /rotating the
    //camera upside-down wouldn't require this!:
        for (int y=0;y<h;y++){
          memcpy(target.getRowArray()[h-y-1],&buf[y*w*3],w*3);
        }
    port.write();
...
{		
    ...
    odeinit._wrld->waitOBJ2 = 0;
    odeinit._wrld->S_lablOBJNUM = 0;
	odeinit._wrld->objManText = 49;
	dInitODE(); 
