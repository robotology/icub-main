...
void iCub::contrib::primateVision::AttnServer_Stereo::run(){
  ...
  motion_request.content().suspend = 100; //these moves are high-piriorty, avoid conflicts.
  ...
  xres              = ippiMalloc_32f_C1(srcsize.width,srcsize.height, &psbxres);
  ...
  while (1){
    ...
    if (use_tsb){inBot_tsb_l = inPort_tsb_l.read(false);}
    if (use_ior){inBot_ior_l = inPort_ior_l.read(false);}
    inBot_sal_r = inPort_sal_r.read(false);
    if (use_tsb){inBot_tsb_r = inPort_tsb_r.read(false);}
    if (use_ior){inBot_ior_r = inPort_ior_r.read(false);}
    inBot_rec_l = inPort_rec_l.read();
    ...
    //LEFT
    inBot_sal_l = inPort_sal_l.read();//false    
    ...
    if (!got_sal_r && inBot_sal_r!=NULL){
      ...
      ippiConvert_8u32f_C1R(sal_in_r,psb_in,attn_r,psb_32f,srcsize); 
      got_sal_r=true; 
    }
...
#if INFO
      printf("MAX_FIXVAL_L: %f  @(x,y): (%d,%d)\n",fix_val_l,fix_x,fix_y_l);
#endif
      ippiMaxIndx_32f_C1R(attn_r,psb_32f,srcsize,&fix_val_r,&fix_x_r,&fix_y_r);
#if INFO
      printf("MAX_FIXVAL_R: %f  @(x,y): (%d,%d)\n",fix_val_r,fix_x,fix_y_r);
#endif
	...
	if (lr_win==ALEFT){
	  ...
	  xcheck_ok = crosscheck(rec_in_l,targ_x_l,targ_y_l,rec_in_r,psb_in,&targ_x_r,targ_y_r);
	}
	else{
	  //do stereo cross-checking for des l: ONLY DO THIS IF NO STEREO MAP! (LATER)
	  targ_x_r = fix_x_r;
	  ...
	  xcheck_ok = crosscheck(rec_in_r,targ_x_r,targ_y_r,rec_in_l,psb_in,&targ_x_l,targ_y_l);
	}
	
	//if cross-check passed, Motion!!
	if (!xcheck_ok){
	  lr_win = ANONE;
#if INFO
	...
	if (filter_saccades){
	  ...
	  for (int i=0;i<num_v;i++){
	    past_fix_x_l[i] += targ_x_l-srcsize.width/2;
	    past_fix_y_l[i] += targ_y_l-srcsize.height/2;
	    past_fix_x_r[i] += targ_x_r-srcsize.width/2;
	    past_fix_y_r[i] += targ_y_r-srcsize.height/2;
	  }
...
bool iCub::contrib::primateVision::AttnServer_Stereo::crosscheck(Ipp8u*ima,int x_a,int y_a,Ipp8u*imb,int psbx,int*x_b,int y_b){
  ...
  if ( (x_a-xtsize.width/2-1  > 0)              && 
    ...
    //cut out template from a image at max fix point,
    //search for it in b image across full image in band around a im fix coords:
    int res = ippiCrossCorrSame_NormLevel_8u32f_C1R(&imb[0 + (y_b-xisize.height/2)*psbx],
					  psbx,
					  xisize,
    ...
    //peek:  
    ippiMaxIndx_32f_C1R(xres,psbxres,srcsize,&max_32f,x_b,&foo);
    
#if INFO
    printf("XCHECK - %d ax:%d ay:%d bx:%d by:%d by_foo:%d res:%f\n",res,x_a,y_a,*x_b,y_b,foo-xisize.height/2,max_32f);
#endif
