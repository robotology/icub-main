...
const double START_POSITION_ARM[]={-30, 50, 0,  -10, 0, 0, 0, 0, 0, 0, 0, 0, 60, 
-60, 0};
const double STOP_POSITION_ARM[]={-30,  0,  0,  -10, 0, 0, 0, 0, 0, 0, 0, 0, 60, 
-60, 0};
...
const int CLOSED_LOOP_RATE = 33;
const double CLOSED_LOOP_THRESHOLD = 0;
const double CL_GAIN=0.6;
const double CL_MAX_SPEED=10;
const int CL_TIMEOUT=30; 
FILE *OUTPUT_FILE = fopen("Data_ClosedVSOpenLoopImagePlaneErrors8.txt","w");
...
void initFile()
{
	fprintf(OUTPUT_FILE, "targetRight_x[pixels] targetRight_y[pixels] targetLeft_x[pixels] targetLeft_y[pixels] ");
	fprintf(OUTPUT_FILE, "dQ0[deg/s] dQ1[deg/s] dQ2[deg/s] dQ3[deg/s] ");
	fprintf(OUTPUT_FILE, "Q0[deg] Q1[deg] Q2[deg] Q3[deg] ");
	fprintf(OUTPUT_FILE, "J11[deg/s] J12[deg/s] J13[deg/s] ");
	fprintf(OUTPUT_FILE, "J21[deg/s] J22[deg/s] J23[deg/s] ");
	fprintf(OUTPUT_FILE, "J31[deg/s] J32[deg/s] J33[deg/s] ");
	fprintf(OUTPUT_FILE, "J41[deg/s] J42[deg/s] J43[deg/s] ");
	fprintf(OUTPUT_FILE, "\n");
}

void writeFile(double val)
{
	fprintf(OUTPUT_FILE, "%.6f ", val);
}
...
class CollectorPortHandler:public RateThread
{
private:
	Semaphore mutex;
    ...
    bool timedOut;
public:
	CollectorPortHandler():RateThread(CLOSED_LOOP_RATE/2), prev(0), now(0), spokeTime(0), spoke(false)
	{
		Datum.addInt(128);
		Datum.addInt(128);
		Datum.addInt(0);
...
velArm(0) 
	...
	void run()
	{
		...
		YVector jacobian_s(3);
		YMatrix jacobian_V(3,3);
		YMatrix jacobian_U(4,3);
		YMatrix jacobian_S(3,3);
		YMatrix jacobian_Spinv(3,3);
		YMatrix jacobian_pinv(3,4);
		YMatrix jacobian_pinv_diff(3,4);
		YMatrix eye(4,4);
		YVector X(3);
		YVector W(4);
		W(1) = 0;	W(2) = 0;	W(3) = 5;	W(4) = 0;	
		...
		X(1)=botRight.get(0).asDouble();
		X(2)=botRight.get(1).asDouble();
		X(3)=botLeft.get(0).asDouble();
		...
		jacobian_U = jacobian;
		SVD( jacobian_U, jacobian_s, jacobian_V);
		for (c=1;c<=3; c++)
		{
			for(r=1;r<=3;r++)
			{
				if (r==c)
					jacobian_Spinv(r,c) = 1/jacobian_s(c);
				else
					jacobian_Spinv(r,c) = 0;
			}
		}


		jacobian_pinv = jacobian_V*jacobian_Spinv*jacobian_U.Transposed();
		jacobian_pinv_diff = (jacobian_pinv * jacobian * jacobian_pinv) - jacobian_pinv;
		//fprintf(stderr, "Pinv error --> %.3lf\n", jacobian_pinv_diff(1,2));
		...
		{
			if ((count%400) > 200 && (count%400) < 400)
				dQ=jacobian*X + (eye - jacobian * jacobian_pinv)*W;
				//dQ=(eye - jacobian * jacobian_pinv)*W;
			else
				dQ=jacobian*X - (eye - jacobian * jacobian_pinv)*W;
				//dQ= - (eye - jacobian * jacobian_pinv)*W;
		}
		...
		else ///////////////// control loop
		{
			...
			writeFile(X(2));
			writeFile(X(3));
			writeFile(botLeft.get(1).asDouble());
			writeFile(dQ(1));		//shoulder first joint
			writeFile(dQ(2));		//shoulder second joint
			writeFile(dQ(3));		//shoulder third joint
			writeFile(dQ(4));		//elbow
			writeFile(Q(1));		//shoulder first joint
			writeFile(Q(2));		//shoulder second joint
			writeFile(Q(3));		//shoulder third joint
			writeFile(Q(4));		//elbow


			for(int r=1;r<=4;r++)
				for (int c=1;c<=3; c++)
					writeFile(jacobian(r,c));
		...
		if(clState==1)
		{
			fprintf(stderr, "CL succesfully terminated\n");
			Time::delay(1);
			velArm->velocityMove(0,0);
			velArm->velocityMove(1,0);
			velArm->velocityMove(2,0);
		...
		double t2 = Time::now();
        if (t2-t1>0.033)
			fprintf(stderr, "WARNING: TimeLoopThread --> %lf\n", t2-t1);

        if (dT>CLOSED_LOOP_RATE*3.0/2.0)
			fprintf(stderr, "WARNING: Thread period --> %lf\n", dT);
	}
...
public:
	...
	void fixedTimeMove(double *startPositionsArm, double  *cmdPositionsArm, double *startPositionsHead, double  *cmdPositionsHead, double cmdTime);
	bool waitFixation();
	void resetArm();
    ...
    void threadRelease()
	{
		fprintf(stderr, "FeedFwdArmMover Thread --> Stopping thread\n"); 
		closedLoop->stop();
		...
		fprintf(stderr, "FeedFwdArmMover Thread --> Thread stopped\n"); 
	}
...
///// some functions
//
void feedfwdArmMover::fixedTimeMove(double* startPositionsArm,  double* 
cmdPositionsArm, double* startPositionsHead, double* cmdPositionsHead, double cmdTime)
{
	//arm movement
