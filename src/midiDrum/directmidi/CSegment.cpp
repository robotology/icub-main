...
HRESULT CSegment::GetDefaultTempo(DMUS_TEMPO_PARAM* tempo)
{
	TCHAR strMembrFunc[] = _T("CSegment::GetDefaultTempo()");
	DM_ASSERT(strMembrFunc,__LINE__,m_pSegment==NULL);
	m_pSegment->GetParam(GUID_TempoParam,0xFFFFFFFF ,DMUS_SEG_ANYTRACK,0,NULL, tempo);
	return S_OK;
}
