...
int main(int argc, char *argv[])
{
    ...
    EpisodicMemory episodicMemoryModule;	// instantiate module
 
    /* prepare and configure the resource finder */

    ResourceFinder rf;
    rf.setVerbose(true);
    rf.setDefaultConfigFile("episodicMemory.ini");       //overridden by --from parameter
    rf.setDefaultContext("episodicMemory/conf");         //overridden by --context parameter
    rf.configure("ICUB_ROOT", argc, argv);

    /* run the module: runModule() calls configure first and, if successful, it then runs */

    episodicMemoryModule.runModule(rf);
