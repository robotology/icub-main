...
bool fingerImageThread::threadInit(){
	...
	iCanBus->canIdAdd(0x306);
	iCanBus->canIdAdd(0x307);
...
void fingerImageThread::run(){
	...
	for (unsigned int i=0; i<readMessages; i++)
	{
		if (readBuffer[i].getId()==0x306)
		{
			...
			for (int i=0; i<7; i++) {
				sensor[i] =  (256 - 2 * sensor[i]) - 40;
				if (sensor[i] < 0) sensor[i] = 0;
			}
		}
		else if (readBuffer[i].getId()==0x307)
		{
	...
	int medPointX=160;	
	redraw = false;
	addRectangle(*image2,PixelRgb(sensor[5],sensor[5],0),medPointX,45,25,25);
	addRectangle(*image2,PixelRgb(sensor[3],sensor[3],0),medPointX-78,97,25,25);
	addRectangle(*image2,PixelRgb(sensor[4],sensor[4],0),medPointX-26,97,25,25);
	addRectangle(*image2,PixelRgb(sensor[6],sensor[6],0),medPointX+26,97,25,25);
	addRectangle(*image2,PixelRgb(sensor[7],sensor[7],0),medPointX+78,97,25,25);
	addRectangle(*image2,PixelRgb(sensor[2],sensor[2],0),medPointX-78,149,25,25);
	addRectangle(*image2,PixelRgb(sensor[1],sensor[1],0),medPointX-26,149,25,25);
	addRectangle(*image2,PixelRgb(sensor[9],sensor[9],0),medPointX+26,149,25,25);
	addRectangle(*image2,PixelRgb(sensor[8],sensor[8],0),medPointX+78,149,25,25);
	addRectangle(*image2,PixelRgb(sensor[0],sensor[0],0),medPointX-26,201,25,25);
	addRectangle(*image2,PixelRgb(sensor[10],sensor[10],0),medPointX+26,201,25,25);
	addRectangle(*image2,PixelRgb(sensor[11],sensor[11],0),medPointX+78,201,25,25);
	redraw = true;
}
