...
If used in YARP mode, this application creates a port /skinGui/<robotPart>:i, where <robotPart> correspondes to the value
of the parameter robotPart contained in the .ini configuration file. For example, if $ICUB_ROOT/app/iCubSkinDemo/lefthand.ini
is used, then a /skinGui/left_hand:i port is created. The port must be afterwards connect to the output port of skin output
interface created by iCubInterface. (example: yarp connect /icub/skin/lefthand /skinGui/left_hand:i)
...
The application uses a configuration file (e.g. lefthand.ini) located in $ICUB_ROOT/app/iCubSkinDemo.
You can select the configuration file used by the application using the --from <filename> parameter.
