...
//*********************************************************************************
void button1_click (GtkButton *button,	gpointer   user_data)
{
	STOP_TIMER
	in.len=8;
	in.d0=0;
	...
	in.d6=0;
	in.d7=0;
	downloader.send_generic	(boardId, in,out);
	START_TIMER
}

//*********************************************************************************
void button2_click (GtkButton *button,	gpointer   user_data)
{
	STOP_TIMER
	in.len=8;
	in.d0=0;
...
{ 
	...
	char buff[255];
	//  gtk_window_set_title (GTK_WINDOW (window), "GENERIC SLIDER GUI 1.0, unconnected");
	sprintf(buff, "GENERIC SLIDER GUI 1.0, connected to NET:%d, Board:%d", networkId, boardId);
...
*/
	...
	button2   = gtk_button_new_with_mnemonic ("Button2"); 
    ...
    g_signal_connect (button2, "clicked", G_CALLBACK (button2_click),NULL);
	...
	gtk_fixed_put(GTK_FIXED(fixed),button1,c[1]-20,r[5]+40);
	gtk_fixed_put(GTK_FIXED(fixed),button2,c[2]+10,r[5]+40);
