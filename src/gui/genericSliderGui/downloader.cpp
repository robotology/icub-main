...
//*****************************************************************/
int cDownloader::send_generic  (int target_id, packet in, packet &out)
{
	// check if driver is running
    if (m_candriver == NULL)
        {
	...
	// Send transmission command to strain board
    txBuffer[0].setId((0 << 8) + target_id);
    txBuffer[0].setLen(in.len);
    txBuffer[0].getData()[0]= in.d0;
	txBuffer[0].getData()[1]= in.d1;
	txBuffer[0].getData()[0]= in.d2;
	txBuffer[0].getData()[1]= in.d3;
    txBuffer[0].getData()[0]= in.d4;
	txBuffer[0].getData()[1]= in.d5;
    txBuffer[0].getData()[0]= in.d6;
	txBuffer[0].getData()[1]= in.d7;
	...
	for (int i=0; i<read_messages; i++)
	{
		if (rxBuffer[i].getData()[0]==in.d0) 
			{
				out.len = rxBuffer[i].getLen();
				out.d0=rxBuffer[i].getData()[0];
				out.d1=rxBuffer[i].getData()[1];
				out.d2=rxBuffer[i].getData()[2];
				out.d3=rxBuffer[i].getData()[3];
				out.d4=rxBuffer[i].getData()[4];
				out.d5=rxBuffer[i].getData()[5];
				out.d6=rxBuffer[i].getData()[6];
				out.d7=rxBuffer[i].getData()[7];
				return 1;
			}
	}
	out.len=0;
            ...
            if (rxBuffer[i].getData()[0]==CAN_GET_ADDITIONAL_INFO && rxBuffer[i].getLen()==6) 
                {
                    ...
                    for (int j = 0; j< 4; j++)
                        board_info[part*4+j]=rxBuffer[i].getData()[j+2];
                }
        }
