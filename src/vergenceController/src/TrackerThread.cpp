    ...
    */
    ...
    if ( (delayR<TIMEOUT)&&(delayL<TIMEOUT))
    ...
    else if (delayR<TIMEOUT)
        {
            ...
            command[5]=targetR(2)*Kvergence;//*(encoders[5]-10);  //encoders[5] eyes vergence
            command[3]=Ktilt*targetR(1); //targetR(1) goal velocity for tilt axis
            if(!just_eyes){
                command[0]=Kn_tilt*(encoders[3]); //encoders[3] head tilt
    ...
    else if (delayL<TIMEOUT)
        {
            ...
            command[5]=targetL(2)*Kvergence*(encoders[5]-10);
