...
bool IcubControlModule::open(Searchable& config){
	...
	if (!iCubActions.open(actiondefs_props, sequence_directory)) {
	...
	IhaDebug::pmesg(DBGL_STATUS1,"NUM_ACTIONS defined : %d\n",NUM_ACTIONS);
	...
	if (config.check("connect_to_expression")) {
		if (connectToParam(config,"connect_to_expression",expressionRawInPortName.c_str(), 0.25, this,true)) {
            IhaDebug::pmesg(DBGL_INFO,"Connected to Expression\n");
        } else {
            ok = false;
        }
	}
	...
	for (int part=0;part<numParts;part++) {
		if (active[part]) {
			...
			fprintf(stderr,partconfig.toString());
			fprintf(stderr,"\n");
			...
			IhaDebug::pmesg(DBGL_STATUS1,"%s returns %d axes\n", partConfigName[part].c_str(),numJoints[part]);

			numJointsTotal+=numJoints[part];
		}
	}

    IhaDebug::pmesg(DBGL_DEBUG2,"Allocate arrays. numParts=%d\n",numParts);
	...
	IhaDebug::pmesg(DBGL_STATUS1, "Starting Encoders Output Thread\n");
...
bool IcubControlModule::updateModule(){
    IhaDebug::pmesg(DBGL_DEBUG2,"IcubControlModule::updateModule() called()\n");
    if (commandPort.isWriting()) {
        IhaDebug::pmesg(DBGL_DEBUG2,"Port Still writing.\n");
        Time::delay(0.1);
        return true;
    }
    ...
    {
        IhaDebug::pmesg(DBGL_DEBUG2,"Got command %s\n",cmd.toString().c_str());

        // the current action
        IhaDebug::pmesg(DBGL_DEBUG2,"Current Action : %d\n",currentAction);
        double st=Time::now();
        ...
        if (cmd.size() > 0) {
            ...
            if (!sendAction(currentAction)) {
                ...
                IhaDebug::pmesg(DBGL_DEBUG2,"Action %d took %f seconds\n",currentAction,Time::now()-st);
            }
            IhaDebug::pmesg(DBGL_DEBUG2,"Sending ACK\n");
            commandPort.reply(response);
        }
    } else {
        IhaDebug::pmesg(DBGL_DEBUG2,"No command\n");
        Time::delay(0.1);
    }
...
bool IcubControlModule::doPositionMove(IPositionControl *pos, int joint, int ref, int speed) {
	...
	IhaDebug::pmesg(DBGL_DEBUG2,"setRefSpeed(%d,%d)\n",joint,sp);
	if (!pos->setRefSpeed(joint,sp)) {
		ACE_OS::fprintf(stderr,"Error setting ref speed\n");
		return false;
	}
	IhaDebug::pmesg(DBGL_DEBUG3,"Joint %d Ref %d Speed %d\n",joint,ref,speed);
...
bool IcubControlModule::doPositionMove(IPositionControl *pos, int joint, int ref) {
	if (ref==JOINTREF_UNDEF) return true;
	IhaDebug::pmesg(DBGL_DEBUG3,"Joint %d Ref %d \n",joint,ref);
	if (!pos->positionMove(joint,ref)) {
		ACE_OS::fprintf(stderr,"Error in positionMove\n");
		return false;
	}
...
bool IcubControlModule::doPositionMove(int joint, int ref, int speed) {
	...
	for (int part=0; part<numParts; part++) {
		if (active[part] && joint<index+numJoints[part]) {
			IhaDebug::pmesg(DBGL_DEBUG2,"%s : Move joint %d to %d Speed %d \n", partName[part].c_str(), joint-index, ref, speed);
...
bool IcubControlModule::doSetSpeed(int joint, int speed) {
	...
	for (int part=0; part<numParts; part++) {
		if (active[part] && joint<index+numJoints[part]) {
			IhaDebug::pmesg(DBGL_DEBUG2,"%s : Set speed joint %d to %d \n", partName[part].c_str(), joint-index, speed);
...
bool IcubControlModule::waitForCompletion() {
	...
	IhaDebug::pmesg(DBGL_DEBUG1,"motors to move (M=move, F=fault):\n             ");
	for (int part=0;part<numParts;part++) {
		if (active[part]) {
			IhaDebug::pmesg(DBGL_DEBUG1," %s:",partName[part].c_str());
			for (int j=0;j<numJoints[part];j++) {
				IhaDebug::pmesg(DBGL_DEBUG1,"%s",motorFault[part][j]?"F":(motorMoving[part][j]?"M":partMoving[part]?".":" "));
			}
		}
	}
	...
	while (!complete) {
		...
		for (int part=0;part<numParts;part++) {
			if (active[part]) {
				IhaDebug::pmesg(DBGL_DEBUG1," %s:",partName[part].c_str());
				for (int j=0;j<numJoints[part];j++) {
					IhaDebug::pmesg(DBGL_DEBUG1,"%s",partMoving[part]==false?" ":motorMoving[part][j]?"T":".");
				}
			}
		}
		IhaDebug::pmesg(DBGL_DEBUG1,"\n");
		...
		if (Time::now()-st > motor_motion_timeout/1000) {
			IhaDebug::pmesg(DBGL_STATUS1,"Motor Motion Timout\n");
			for (int part=0;part<numParts;part++) {
				if (active[part] ) {
					for (int j=0;j<numJoints[part];j++) {
						motorFault[part][j]=motorMoving[part][j];
						if (motorFault[part][j]) IhaDebug::pmesg(DBGL_STATUS1,"%s:%d faulty.\n",partName[part].c_str(),j);
					}
				}
			}
			return true;
		...
		if (!complete) {
			Time::delay(0.05);
			if (IhaDebug::getLevel()<DBGL_DEBUG1) IhaDebug::pmesg(DBGL_STATUS1,"*");
		}
	}
	return true;
}
...
bool IcubControlModule::runSequenceParallel(string seq) {
    IhaDebug::pmesg(DBGL_DEBUG2,"runSequenceParallel: %s\n",seq.c_str());
	bool running = true;
	string str;
	...
	while (ss >> str && running) {
		if (str=="POS") 
		{
			...
			ss >> ref;
			ss >> speed;
			IhaDebug::pmesg(DBGL_DEBUG3,"pos request  %d %d %d\n",joint, ref, speed);
		...
		else if (str=="BLK") 
		{
			...
			// so now send it
			if (multiMovePending) {
				IhaDebug::pmesg(DBGL_DEBUG1, "Sending multi pos move\n");
				...
				IhaDebug::pmesg(DBGL_DEBUG2,"Multi took %f seconds\n",Time::now()-st);

				multiMovePending=false;
			}

			IhaDebug::pmesg(DBGL_DEBUG1,"Blocking\n");
			double st=Time::now();
			...
			IhaDebug::pmesg(DBGL_DEBUG1,"BLK took %f seconds\n",Time::now()-st);

		} 
		else 
		{
			IhaDebug::pmesg(DBGL_INFO,"Incorrect string: %s\n",seq.c_str());
			running=false;
		}
	}
...
bool IcubControlModule::sendAction(int act) {
    ...
    if (act==1 || act==2 || act==16) { // this needs to change to use BSet5
        IhaDebug::pmesg(DBGL_STATUS2,"sendExpression %d started.\n",act);
        retval = sendExpression(act);
        IhaDebug::pmesg(DBGL_STATUS2,"sendExpression %d complete. %s\n",act, retval?"":"FAILED");
    }
    if (act < 0 || act > iCubActions.getNumActions()) {
        IhaDebug::pmesg(DBGL_STATUS2,"Action %d not configured.\n",act);
        return false;
    }
    ...
    for (int part=0;part<numParts;part++) {
        if (active[part]) {
            if (!enc[part]->getEncoders(encs[part])) {
                ...
                IhaDebug::pmesg(DBGL_DEBUG2,"Got part=%d=%s encoders.\n",part,partName[part].c_str());
            }
        }
    }
    return encs;
...
void iCub::iha::EncodersOutputThread::run() {
    ...
    if (IhaDebug::getLevel()==DBGL_STATUSLINE) {
        IhaDebug::pmesg(DBGL_STATUSLINE,"TS: %d Action: %f\r",ms,ca);
    } else {
        IhaDebug::pmesg(DBGL_STATUS1,"TS: %d Action: %f\n",ms,ca);
    }
        
    IhaDebug::pmesg(DBGL_DEBUG2,"EncodersOutputThread::run() took %f ms.\n",1000*(Time::now()-start));
}
...
void IcubControlModule::debugPrintArrays() {
	...
	for (int part=0;part<numParts;part++) {
		if (active[part]) {
            ACE_OS::fprintf(stderr,"Part %d. Joints %d : \n",part,numJoints[part]);
             for (int j=0;j<numJoints[part];j++) {
