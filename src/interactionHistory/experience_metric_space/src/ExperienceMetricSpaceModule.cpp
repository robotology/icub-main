...
bool ExperienceMetricSpaceModule::open(Searchable& config){
	...
	reward_index = config.find("reward_index").asInt();
	action_index = config.find("action_index").asInt();
...
bool ExperienceMetricSpaceModule::updateModule(){
	...
	// get reward value
	last_value = value;
	value=b->get(reward_index+1).asDouble();
	// and the action 
	last_act = act;
	act = b->get(action_index+1).asInt();
