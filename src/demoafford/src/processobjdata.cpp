...
int processobjdata::classifyshape(double* shapehist)
{
  //	if( (shapehist[2]>0.75) && (shapehist[4]>0.65))
  if ( (shapehist[4]>0.60))
		return 0;
	else
		return 1;
}
double* processobjdata::getcolorhist(double *data)
{
	return (data+7);
}

double* processobjdata::getshapehist(double *data)
{
	return (data+23);
}
