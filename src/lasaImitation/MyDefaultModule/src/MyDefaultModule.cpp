...
int main(int argc, char *argv[])
{
  ...
  if(!yarp.checkNetwork())
      return 0;
  MyDefaultModule module;

  return module.runModule(argc,argv);
}


MyDefaultModule::MyDefaultModule(){
    mPeriod         = 0.02;
    bIsReady        = false;
    mThread         = NULL;
}
...
bool MyDefaultModule::open(Searchable &s){
    if(bIsReady)
        return true;

    if(!mParams.check("name")){
        mParams.put("name","MyDefault000");
        fprintf(stderr, "No module base name specifed, using <MyDefault000> as default\n");
    ...
    char portName[255];
    snprintf(portName,255,"/MyDefault/%s/rpc",getName().c_str());
    mControlPort.open(portName);
    attach(mControlPort,true);
    
    snprintf(portName,255,"MyDefault/%s",getName().c_str());
    mThread = new MyDefaultThread(int(floor(mPeriod*1000)),portName);
    mThread->start();
...
bool MyDefaultModule::respond(const Bottle& command, Bottle& reply) {
    int  index      = 0;
...
double MyDefaultModule::getPeriod(){
    return 2.0;
}

int MyDefaultModule::runModule(int argc, char *argv[], bool skipFirst){
    mParams.fromCommand(argc, argv);
    return Module::runModule(argc,argv,skipFirst);
}

bool   MyDefaultModule::updateModule() {
    return true;
}
