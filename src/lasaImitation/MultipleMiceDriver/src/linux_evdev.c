...
static int init_mouse(const char *fname, int fd)
{
	...
	if ( (test_bit(relcaps, REL_X)) && (test_bit(relcaps, REL_Y)) && (test_bit(relcaps, REL_Z)) && 
	     (test_bit(relcaps, REL_RX)) && (test_bit(relcaps, REL_RY)) && (test_bit(relcaps, REL_RZ)))
	     mouse->has3D = 1;
	else
		mouse->has3D = 0;
    ...
    char physloc[64];    
    if(ioctl(fd, EVIOCGPHYS(sizeof (physloc)), physloc) == -1){
      memset(physloc,0,sizeof (physloc));
    }
    strcat(mouse->name," <");
    strcat(mouse->name,physloc);
    strcat(mouse->name,">");
