...
/**
 * Modified segments_nearest_points
 */
...
void q_complete(const CVector3_t v, CQuat_t out){
  v_copy(v,out);
  out[3] = sqrt(1-v_squ_length(v));
}


#ifdef DEPRECATED
/**
   derivative of a quaternion with respect to the norm.
   not definite if quaternion is null (q[3]=1)
 */
void q_derivative(const CQuat_t q, CQuat_t out){
 
  float n = v_normalize(q,out);
...
void m_set_v3_column(const CVector3_t v,int i, CMatrix3_t out)
{
    v_copy(v,out+4*i);    
}
...
istream& operator>>(istream& in,CVector3_t& v){
   in>>v[0]>>v[1]>>v[2];
   return in;
}
...
int segments_nearest_points(const CVector3_t p1,const CVector3_t vec1,
  ...
  f= v_length(vec3)/(v_length(vec1)*v_length(vec2));//sine of angle
  if(f<epsilon){
    //    cout<<"aborting"<<endl;
    return 0;
  }
  ...
  v_add(p1,tmp,tmp1);//cout<<"tmp1= "<<tmp1<<endl;
  v_scale(vec2,k[1],tmp);
  v_add(p2,tmp,tmp2);//cout<<"tmp2= "<<tmp2<<endl;
  v_sub(tmp2,tmp1,tmp);

  *dist=v_length(tmp);
  *nearest_point1 = k[0];//cout<<"k0= "<<k[0];
  *nearest_point2 = k[1];//cout<<"  k1= "<<k[1]<<endl;
  return 1;
}
