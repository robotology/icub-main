...
extern float  _integral[JN] ;			// store the sum of the integral component 
...
{ \
    ...
    { \
    	if ( _ki[j] != 0) \
		{ \
    		_integral[j] =  (float) ((Int32) _pid_limit[j]) - _pd[j]; \
		} \
	} \
	else \
	{\
...
//		{\
//			AS1_printStringEx ("R: "); \
//			AS1_printWord16AsChars(j); \
//			AS1_printStringEx (" "); \
//			_control_mode[j] = MODE_HANDLE_HARD_STOPS; \
//			AS1_printDWordAsChars(_filt_current[j]); \
//			_pid[j] = 0; \
