...
void init_temp_sens (void)
{
 ...
 //////////             CONNECTION TEST   /////////////////
	 ...
	 if (data_register[0] & TEMP_PIN1) 
 		temp_sens_status =TEMPERATURE_STATUS_ERR1;
	 if (data_register[1] & TEMP_PIN2) 
 		temp_sens_status =TEMPERATURE_STATUS_ERR2;
...
bool ResetTempSens_part2(void)
{
 ...
 // delay (430); 
 return !(error_flag[0] || error_flag[1]);
}
 ...
 switch (temp_sens_status)
 {
 	...
 	case TEMPERATURE_STATUS_ERR:
  	...
  	case TEMPERATURE_STATUS_RUNNING+1:
 		...
 		{
 			if (error_flag[0] && error_flag[1]) temp_sens_status = TEMPERATURE_STATUS_ERR;
 			else 
 			{
 				if (error_flag[0]) temp_sens_status = TEMPERATURE_STATUS_ERR1;
 				else temp_sens_status = TEMPERATURE_STATUS_ERR2;
 			}
 			return temp_sens_status;
		}
		temp_sens_status++;
