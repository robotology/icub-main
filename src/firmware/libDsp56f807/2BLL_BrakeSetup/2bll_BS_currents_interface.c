...
Int32 _max_allowed_current[4] = {6000000,6000000,6000000,6000000};	/* limit on the current in micro-ampere*/							
...
//	#warning "_current now is Iss the Imot=Iss/deltaPWM;"
	...
	switch (jnt)
	{
		...
		case 1: AD_getChannel16B (1, &temp); break;			
	}
	if ( temp < 3328)	temp = 0;
	else temp=temp-3328;
    ...
    _current[jnt]=  (temporary * _conversion_factor[jnt]);//* ((Int32)MAX_DUTY/ DutyCycle[jnt].Duty);//see BLP and BLL manual
