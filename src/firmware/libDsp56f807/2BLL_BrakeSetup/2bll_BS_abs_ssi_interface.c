...
UInt16 get_absolute_real_position_abs_ssi(byte jnt)
{
	...
	for(i=0;i<18;i++)
	{
		...
		{
		 	status.wordbb[i-12]=getRegBits(GPIO_E_DR,GPIO_E6);	 	
		}
	}
...
void get_status_abs_ssi(bool* s_ocf,bool* s_cof,bool* s_lin, bool* s_inc, bool* s_dec)
{
	(*s_ocf)= status.status_ocf;
	(*s_cof)= status.status_cof;
	(*s_lin)= status.status_lin;
	(*s_inc)= status.status_inc;
	(*s_dec)= status.status_dec;
}
...
//	get_absolute_real_position_abs_ssi(jnt);
	...
	if ((status.status_inc==1) && (status.status_dec==1) && (status.status_ocf==1) && (status.status_cof==1) && (status.status_lin==1) )
	{
	return ERR_ABS_SSI;	
	}
