...
Int32 _current[4] =		 {0,0,0,0};					/* current in mA*/
Int32 _current_old[4] =  {0,0,0,0};					/* current at t-1*/
Int32 _filt_current[4] = {0,0,0,0};     			/* filtered current through the transistors*/
Int32 _max_allowed_current[4] = {7000,7000,7000,7000};	/* limit on the current in micro-ampere*/							
...
//	#warning "_current now is Iss the Imot=Iss/deltaPWM;"
	...
	if (DutyCycle[jnt].Duty<=(MIN_DUTY+DEAD_TIME)+20) //20 is a value for do not have a current too big for low duty cycles 
	{
		_current[jnt]=0.25;
	}
	else
