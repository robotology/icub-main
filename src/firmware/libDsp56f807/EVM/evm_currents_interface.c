...
dword _filt_current[4] = {0,0,0,0};     			/* filtered current through the transistors*/
dword _max_allowed_current[4] = {4000000,4000000,4000000,4000000};	/* limit on the current in micro-ampere*/							
float _conversion_factor[4] = {0f,0f,0f,0f} ;		/* limit on the current as set by the interface (later converted into the filter parameter) */
...
void compute_filtcurr(byte jnt)
{
	...
	*/
	...
	_filt_current[jnt] = 0.9886 * filt_current_old[jnt] + 5.7 * (_current_old[jnt] + _current[jnt]);
