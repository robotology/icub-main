...
#include "asc.h"
#include "can1.h"
#include "2bll_pwm_reload_sync.h"
...
void broadcast_strain_debug_info(void)
{
	...
	Int32 commut0=0;
		...
		{
		    _canmsg.CAN_data[2] = BYTE_H(DutyCycle[0].Duty);
			_canmsg.CAN_data[3] = BYTE_L(DutyCycle[0].Duty);		
	    }
		else
		{
		...
		*/
		...
		{
		    _canmsg.CAN_data[2] = BYTE_H(DutyCycle[1].Duty);
			_canmsg.CAN_data[3] = BYTE_L(DutyCycle[1].Duty);		
	    }
		else
		{
...
// BCAST_STATUS
	...
	if ((broadcast_mask & (1<<(CAN_BCAST_STATUS-1))) && _counter == 2)
	{
		...
		if (getHallStatus(0))
		{
			_canmsg.CAN_data[0] |= ((getHallStatus(0) &0x1)<<4);
			_canmsg.CAN_data[0] |= ((getHallStatus(0) &0x2)<<5);
			_canmsg.CAN_data[0] |= ((getHallStatus(0) &0x4)<<5);
			setHallStatus(0,0);
			send=true;
		}
    	...
    	//Hall axis 1 reading error
   
  		if (getHallStatus(1))
		{
			_canmsg.CAN_data[2] |= ((getHallStatus(1) &0x1)<<4);
			_canmsg.CAN_data[2] |= (((getHallStatus(1)&0x2))<<5);
			_canmsg.CAN_data[2] |= (((getHallStatus(1)&0x4) )<<5);
			setHallStatus(1,0);
			send=true;
		}
