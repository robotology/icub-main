			...
			{
				{
			    	...
			    	switch (_canmsg.CAN_data[0]) 
			    	{
						...
						case 0xFF:
							...
							can_printf("PWM DISABLED ON CANLOADER REQUEST");
							break;								
...
void can_send_broadcast(void)
{
	...
	if ((broadcast_mask & (1<<(CAN_BCAST_STATUS-1))) && _counter == 2)
	{
		...
		if (_fault[0] == 0 && iretval != 0)
		{
			...
			_canmsg.CAN_data[0]=(iretval>>8) | (iretval>>9) | (iretval>>10) | (iretval>>12); 	
			_canmsg.CAN_data[1]= 0; //not used so far			
		#ifdef DEBUG_CAN_MSG	
				can_print_string("FAULT AX0");	
		#endif
		...
		if (_fault[1] == 0 && iretval != 0)
		{
			...
			_canmsg.CAN_data[2]=((iretval>>8) & 0x01)  | ((iretval>>9) & 0x02) | ((iretval>>10) & 0x04) | ((iretval>>12) & 0x04); 				
			_canmsg.CAN_data[3]= 0; //not used so far
		#ifdef DEBUG_CAN_MSG	
				can_print_string("FAULT AX1");	
		#endif
			// new fault, copy iretval in _fault.
			_fault[1] = iretval;
			send = true;
		...
		if (getCanTXstatus())	setCanTXstatus(0);
		
			send=true;
		#ifdef DEBUG_CAN_MSG	
				can_print_string("CAN_ERR");	
		#endif	
		}
		...
		if (mainLoopOVF)
		{
			_canmsg.CAN_data[5] |= 0x01;
			send=true;
		#ifdef DEBUG_CAN_MSG	
			can_print_string("MAIN OVF");	
		#endif	
		}
...
void set_can_masks()
{
	...
	if (can_channels==2)
	{
		...
		if (VERSION == 0x0113)
			create_F_M(&filter2, &mask2,0b001,0x0b, CAN_BCAST_POSITION,   0b111, 0x0,  0b1111);
		else
