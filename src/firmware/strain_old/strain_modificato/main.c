...
{    
  ...
  // verify if the channel is among active ones 
  if( BoardConfig.EE_AN_ActiveChannels[BoardConfig.EE_AN_SelectedChannel] !=0)
  // if( BoardConfig.EE_AN_SelectedChannel == 4)
  {
    // channel active
    SetDACGetADC(PDM_NORMAL, BoardConfig.EE_AN_ChannelOffset[BoardConfig.EE_AN_SelectedChannel], &adc);

    //
    // store LPF samples or row ADC values
//@@@@
//#define DO_IIR_LPF 
	adc-=0x7FFF;
	if (filter_enable)
  ...
  {
    // inactive
    BoardConfig.EE_AN_ChannelValue[BoardConfig.EE_AN_SelectedChannel]=0;     
  }
  ...
  if (BoardConfig.EE_AN_SelectedChannel == 5)
    ...
    BoardConfig.EE_AN_SelectedChannel++;
