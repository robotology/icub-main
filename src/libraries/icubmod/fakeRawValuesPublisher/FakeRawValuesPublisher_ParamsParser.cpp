...
bool FakeRawValuesPublisher_ParamsParser::getParamValue(const std::string& paramName, std::string& paramValue) const
{
    if (paramName =="name")
    {
        paramValue = m_name;
    ...
    if (paramName =="njomos")
    {
        paramValue = std::to_string(m_njomos);
        return true;
    }
...
std::string FakeRawValuesPublisher_ParamsParser::getConfiguration() const
{
