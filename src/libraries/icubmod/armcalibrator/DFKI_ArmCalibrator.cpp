...
bool DFKI_ArmCalibrator::open (yarp::os::Searchable& config)
{
    Property p;
...
bool DFKI_ArmCalibrator::calibrate(DeviceDriver *dd)
{
    fprintf(stderr, "Calling DFKI_ArmCalibrator::calibrate: \n");
    abortCalib=false;
...
*/
    ...
    iAmps->enableAmp(2);
    iPids->enablePid(2);
	int secondSetOfJoints[] = { 5, 10, 12, 14, 15};    /*2,*/
	for (k =0; k < 5; k++)   /*6*/
...
void DFKI_ArmCalibrator::calibrateJoint(int joint)
{
    iCalibrate->calibrate2(joint, type[joint], param1[joint], param2[joint], param3[joint]);
}

bool DFKI_ArmCalibrator::checkCalibrateJointEnded(int joint)
{
...
void DFKI_ArmCalibrator::goToZero(int j)
{
    if (abortCalib)
        return;
...
void DFKI_ArmCalibrator::checkGoneToZero(int j)
{
    // wait.
    bool finished = false;
    int timeout = 0;
...
bool DFKI_ArmCalibrator::checkGoneToZeroThreshold(int j)
{
    // wait.
    bool finished = false;
    int timeout = 0;
...
bool DFKI_ArmCalibrator::park(DeviceDriver *dd, bool wait)
{
	int nj=0;
    bool ret=false;
    abortParking=false;
	...
	int timeout = 0;
    fprintf(stderr, "ARMCALIB::Calling DFKI_ArmCalibrator::park() \n");
    ...
    if (wait)
    {
		...
		if(!done)
		{
			for(int j=0; j < nj; j++)
			{
				...
				{
					if (!done)
						fprintf(stderr, "DFKI_ArmCalibrator::park(): joint %d not in position ", j);
				}
				else
					fprintf(stderr, "DFKI_ArmCalibrator::park(): joint %d did not answer ", j);
			}
		}
    }
...
bool DFKI_ArmCalibrator::quitCalibrate()
{
...
bool DFKI_ArmCalibrator::quitPark()
{
