...
//#include "EOtheNVsCfgDeviceRem00_hid.h" // to see a single variable ....
...
//static EOVtheNVsCfg *s_thecfg = NULL;
...
//static uint16_t tmp08 = 0;



uint8_t  pkt2tx_data[128] =  {0};
uint16_t pkt2tx_size = 0;
...
volatile uint32_t aaa32;
volatile uint16_t aaa16;
volatile uint8_t  aaa08;

EOnetvar *nvInp08 = NULL;
EOnetvar *nvInp16 = NULL;

uint8_t  *pInp08 = NULL;
uint16_t *pInp16 = NULL;


EOnetvar *nvTMP = NULL;

EOvportCfg *vportcfg = NULL;

eOropconfig_t ropcfg = {.confrqst = eobool_true, .timerqst = eobool_true, .plussign = eobool_true, .plustime = eobool_true};
uint32_t sign = 0x21000012;
uint64_t time = 0xaaaabbbbccccdddd;


void test_eonetvar_Init(void)
{
...
//
//    s_smartnode_pktmake_ask_vportRO(ipaddr_simplenode00, pkt2tx_data);
//
...
//
//    reply_avail = s_simplenode_pktproc_from(ipaddr_smartnode, pkt2tx_data, sizeof(pkt2tx_data));
//     
//    // the smart node processes a packet from .
//    if(eobool_true == reply_avail)
//    {
//        s_smartnode_pktproc_from(ipaddr_simplenode00, pkt2tx_data, sizeof(pkt2tx_data)); 
//    }
//
//
//
//
//    eo_agent_OutROPinit(eo_agent_GetHandle(), eo_common_ipv4addr(10, 10, 1, 0),
//                        eo_ropcode_ask, nvIDrem00dev_vportROnly__dat, ropcfg, rop0);
//
//    eo_agent_OutROPfill(eo_agent_GetHandle(), rop0, &sign, &time);
//
//
//    eo_former_GetStream(eo_former_GetHandle(), rop0, pkt2tx_data, &pkt2tx_size, &ipaddr);
//
//    // ok... now the smart node should transmit the pkt to ipaddr 10.1.1.0
//
//
//
//    // now i act as a normal node: parse the packet, then process the rop received from 10.10.10.1
//
//    eo_parser_GetROP(eo_parser_GetHandle(), pkt2tx_data, sizeof(pkt2tx_data), eo_common_ipv4addr(10, 10, 1, 1), rop, &tmp16);
//
//
...
//    }
//
//    // and now the normal node should send the ack/nack pkt to ipaddr 10.1.1.1
//
//
//    // now i am the smart node whcih has received teh pkt from 10.1.1.0: i get the rop
//    eo_parser_GetROP(eo_parser_GetHandle(), pkt2tx_data, sizeof(pkt2tx_data), eo_common_ipv4addr(10, 10, 1, 0), rop, &tmp16);
//
//
//    // i am the smart node whcih receives a reply of the rop whcih tells the value of teh ROonly vport
//    eo_agent_InpROPprocess(eo_agent_GetHandle(), rop, reply);
