...
/** @file       EOtheNVsCfgNode00LocActions.c
...
#include "EOtheNVsCfgNode00LocActions_hid.h" 
...
static void s_node00_loc_action_button_update(void);

static void s_node00_loc_action_signal_vportrodat(void);
...
extern void node00_loc_action_init_nvOBJ_vportRO__cfg(void *p)
{
    ...
    eo_vport_hid_LoadCfg(&node00_loc_vol.vportRO, &node00_loc_vol.vportRO.cfg, eo_nv_ownership_local, (eOipv4addr_t)0);
}
...
extern void node00_loc_action_init_nvOBJ_vportRO__dat(void *p)
{
    ...
    if((1 == node00_loc_vol.globalconfiguration.signalvportro) && (0!= node00_loc_vol.globalconfiguration.withperiod))
    {
        printf("started the timer\n");
        eo_action_SetCallback(act, s_node00_loc_action_signal_vportrodat, eom_callbackman_GetTask(eom_callbackman_GetHandle()));
        eo_timer_Start(tmrvportrodat, eok_abstimeNOW, node00_loc_vol.globalconfiguration.withperiod, eo_tmrmode_FOREVER, act);
    }
}
...
extern void node00_loc_action_init_nvOBJ_button(void *p)
{
    ...
    if((1 == node00_loc_vol.globalconfiguration.acquireinput) && (0 != node00_loc_vol.globalconfiguration.acquisitionperiod))
    {
        printf("start the timer\n");
        s_node00_loc_action_button_update();
       
        eo_action_SetCallback(act, s_node00_loc_action_button_update, eom_callbackman_GetTask(eom_callbackman_GetHandle()));
        eo_timer_Start(tmrbutton, eok_abstimeNOW, node00_loc_vol.globalconfiguration.acquisitionperiod, eo_tmrmode_FOREVER, act);
    }
}
...
extern void node00_loc_action_init_nvOBJ_led00(void *p)
{
    ...
    hal_gpio_init(hal_gpio_portE, (hal_gpio_pin_t)led00.lednumber, hal_gpio_dirOUT, hal_gpio_speed_2MHz);
...
//    uint16_t size;
    ...
    if(1 == node00_loc_vol.globalconfiguration.applyoutput)
    {
//        eo_netvar_Get(nv, eo_nv_strg_volatile, &led00, &size);
        hal_gpio_setval(hal_gpio_portE, (hal_gpio_pin_t)node00_loc_vol.led00.lednumber, (hal_gpio_val_t)node00_loc_vol.led00.outputval);    
    }
}
...
extern void node00_loc_action_init_nvOBJ_led01(void *p)
{
    ...
    hal_gpio_init(hal_gpio_portE, (hal_gpio_pin_t)led01.lednumber, hal_gpio_dirOUT, hal_gpio_speed_2MHz);
...
//    uint16_t size;
    ...
    {
//        eo_netvar_Get(nv, eo_nv_strg_volatile, &led00, &size);
        hal_gpio_setval(hal_gpio_portE, (hal_gpio_pin_t)node00_loc_vol.led01.lednumber, (hal_gpio_val_t)node00_loc_vol.led01.outputval);    
    }
}
...
static void s_node00_loc_action_button_update(void)
{
    extern const EOnetvar node00_loc_nvOBJ_button__inputval;
    hal_gpio_val_t val;
    val = hal_gpio_getval(hal_gpio_portB, hal_gpio_pin7);
    
    eo_netvar_Set(&node00_loc_nvOBJ_button__inputval, &val, eobool_false, eo_nv_upd_dontdo);
}
