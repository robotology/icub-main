...
void upd_core_init(osal_messagequeue_t* txpktqueue, osal_task_t* tskid_ip)
{
    s_txpktqueue = txpktqueue;
    s_tskid_ip = tskid_ip;
}

void upd_core_manage_cmd(datagram_t *rxdgram)
{
    ...
    if (rep != NULL)
    {
        osal_messagequeue_put(s_txpktqueue,(osal_message_t)rep, 10000, osal_callerTSK);  
