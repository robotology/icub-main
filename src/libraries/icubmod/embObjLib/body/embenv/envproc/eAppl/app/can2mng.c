...
// --------------------------------------------------------------------------------------------------------------------
/* @file       can2mng.c
	@brief      This file implements the CAN2 manager task.
...
// --------------------------------------------------------------------------------------------------------------------
static void s_can2mng_callbkp_newDataFromEth(void *arg);


// --------------------------------------------------------------------------------------------------------------------
// - definition (and initialisation) of static variables
// --------------------------------------------------------------------------------------------------------------------
static const char s_task_name[] = "can2task";

static uint8_t my_memorybuff[ETHCAN2_SHDATA_ITEMSIZE];
static uint32_t my_idcan;

static EOtimer                  *s_can2mng_timer;
static EOaction                 *s_can2mng_action_onTimeout;
...
{
    EOMprodConsSharedData_ConsumerCallbackSet(ethCan2_shData, s_can2mng_callbkp_newDataFromEth, (void*)p);
    s_can2mng_timer = eo_timer_New();
    s_can2mng_action_onTimeout = eo_action_New();
    eo_action_SetEvent(s_can2mng_action_onTimeout, (eOevent_t)EVT_CANMNG_TIMEOUT , p);

    CANmng_startup(hal_can_port2, p, &my_idcan);

#ifdef _DEBUG_PRINT_ 
    sprintf(my_deb_string, "can2 completed");
    hal_trace_puts(my_deb_string);  
#endif 
 
}

extern inline void can2mng_task(void *p)
{
...
extern void can2mng_run(EOMtask *tsk, uint32_t evtmsgper)
{
...
#ifdef _DEBUG_PRINT_ 
    sprintf(my_deb_string, "can2 run start");
    hal_trace_puts(my_deb_string);  
#endif     
        if(EVT_CHECK(evt, EVT_CANMNG_STOP)) 
    {
        hal_can_put(hal_can_port2, &frame_stop, hal_can_send_normprio_now);
        res = eo_timer_Stop(s_can2mng_timer);
    ...
    {
        hal_can_put(hal_can_port2, &frame_start, hal_can_send_normprio_now);
        res = eo_timer_Start(s_can2mng_timer, eok_abstimeNOW, 1000, /*eo_tmrmode_ONESHOT*/ eo_tmrmode_FOREVER, s_can2mng_action_onTimeout);
        /* Note: TODO if I can't stop the timer, is it batter i send me the same event..to tray another time??*/  
    ...
    {
        hal_res = hal_can_get(hal_can_port2, &frame_rec, &remaining_recframe);
        frame_rec.id = 2;
        hal_res = hal_can_put(hal_can_port2, &frame_rec, hal_can_send_normprio_now);
    ...
    {
        res = EOMprodConsSharedData_Get(ethCan2_shData, my_memorybuff, 100); //stai attenta!!!! non devi bloccare troppo il task qui se no poi non riceve piu' msg!!!!!!
        if(eores_OK != res )
        {
        ...
        CANmng_parserAndSend_TEST(hal_can_port2, my_memorybuff, my_idcan);
    ...
    if(EVT_CHECK(evt, EVT_CANMNG_TIMEOUT))
    {
        ...
        frame_periodic.id = 2;
        hal_res = hal_can_put(hal_can_port2, &frame_periodic, hal_can_send_normprio_now);
        if(hal_res_OK != hal_res)
        {
...
#ifdef _DEBUG_PRINT_ 
    sprintf(my_deb_string, "can2 run stop");
    hal_trace_puts(my_deb_string);  
#endif 
}
// --------------------------------------------------------------------------------------------------------------------
