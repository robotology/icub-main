...
static const eEmoduleInfo_t s_loader_info __attribute__((at(EENV_ROMSTART+EENV_MODULEINFO_OFFSET))) = 
{
    .info           =
    {
        .entity     =
...
#else
static eEboardInfo_t s_loader_boardinfo =           // 56 BYTES (48 BYTES upto name[16])              
{
    .info           =
    {
        .entity     =
...
int main(void) 
{
    ...
    static const eEmoduleInfo_t *mi;
...
// debug
//    reshal = hal_eeprom_init(hal_eeprom_i2c_01, NULL);
//    reshal = hal_eeprom_erase(hal_eeprom_i2c_01, 6*1024, 2*1024);
//    memset(data, 0, 2048);
//    reshal = hal_eeprom_read(hal_eeprom_i2c_01, 0x1d40, 0x108, data);
//    reshal = reshal;
...
//    memset(data, 0, 2048);
//    reshal = hal_eeprom_read(hal_eeprom_i2c_01, 0x1d40, 0x108, data);
    // write the board-info w/ the unique-id using shalbase
//    s_loader_boardinfo.uniqueid = hal_arch_arm_uniqueid64_get();
//    shalbase_boardinfo_synchronise(&s_loader_boardinfo);
//    {   // use it to verify the correct behaviour of shalbase_
//        const eEboardInfo_t *pp = NULL;
//        shalbase_boardinfo_get(&pp);
//        pp = pp;
//    }

    #warning --> in ems001 add the config of the switch ...
    ...
    shalpart_shal_get(ee_shalPART, &mi);

    mi = mi;

    shalpart_proc_get(ee_procLoader, &mi);

    mi = mi;

//// no need to use shalinfo in here ...
//    // init also shalinfo but only if there is one
...
//    memset(data, 0, 2048);
//    reshal = hal_eeprom_read(hal_eeprom_i2c_01, 0x1d40, 0x108, data);
//    st.type = ee_strg_eeprom;
//    st.size = 0x108;
//    st.addr = 0x1d40;
//    shalbase_storage_clr(&st, 0x100);
//        memset(data, 0, 2048);
//    reshal = hal_eeprom_read(hal_eeprom_i2c_01, 0x1d40, 0x108, data);
//        shalinfo_erase();
//    memset(data, 0, 2048);
...
//    reshal = hal_eeprom_read(hal_eeprom_i2c_01, 0x1d40, 0x108, data);
//    }
//    {   // use it to verify the correct behaviour of shalinfo_
//        const eEboardInfo_t *pp = NULL;
//        const shalinfo_macaddr_t *mm = NULL;
//        const shalinfo_deviceinfo_t *dd = NULL;
//        shalinfo_boardinfo_get(&pp);
//        pp = pp;
//
//        shalinfo_deviceinfo_get(&dd);
//        dd = dd;
//
//        shalinfo_macaddr_get(&mm);
//        mm = mm;
//    }
