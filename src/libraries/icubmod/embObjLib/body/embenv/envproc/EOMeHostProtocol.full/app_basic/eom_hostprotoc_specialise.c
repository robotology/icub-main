...
/* @file       eom_hostprotoc_specialise.c
...
static void (*s_eom_hostprotoc_specialise_on_ropframe_received)(EOpacket*) = NULL;
...
extern const ipal_cfg_t* eom_hostprotoc_specialise_ipal_cfg = &s_ipal_cfg;
...
extern void eom_hostprotoc_specialise_updserver_start(void)
{
    ...
    eo_errman_Assert(eo_errman_GetHandle(), (NULL != s_sem_tx_pkt), "hostprotoc_specialise", "semaphore is NULL");
...
{
    s_eom_hostprotoc_specialise_on_ropframe_received = cbk;
}
...
extern void eom_hostprotoc_specialise_otherthings(void)
{
    ...
    // init environment
    eom_hostprotoc_extra_environment_init();

    // init transceiver
    eom_hostprotoc_extra_transceiver_init();
...
extern void eom_hostprotoc_specialise_transmit(EOpacket *txpkt)
{
    ...
    eo_packet_Addressing_Get(txpkt, &addr, &port);

    ipv4array = (uint8_t*)&addr;

    snprintf(str, sizeof(str)-1, "transmitting to ip = %d.%d.%d.%d, port = %d", ipv4array[0], ipv4array[1], ipv4array[2], ipv4array[3], port); 
    hal_trace_puts(str);

 

    eo_socketdtg_Put(s_skt_rops, txpkt);
...
static void s_udpserver_run(EOMtask *p, uint32_t t)
{
    ...
    if(s_event_from_skt_rops == (evt & s_event_from_skt_rops))
    {
        ...
        if((eores_OK == res) && (NULL != s_eom_hostprotoc_specialise_on_ropframe_received))
        {
            s_eom_hostprotoc_specialise_on_ropframe_received(s_rxpkt);
        }
