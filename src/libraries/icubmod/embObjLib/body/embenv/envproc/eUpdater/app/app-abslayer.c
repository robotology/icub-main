...
// acemor: we dont use flash file system anymore
//    // better to disable irq to let work the flash w/out interrupts
//    hal_sys_irq_disable();
...
//        infosearch.fileid = 0;
//
//        if(fsal_res_NOK_generic == fsal_find("acemor.was.here", &infosearch))
//        {
//            fsal_format(fsal_drive_flash);
//            printf("formatted flash drive\n");
//    
//            fp = fopen("F:acemor.was.here", "w");
//            if(NULL != fp)
//            {
//                len = fwrite(data, 1, sizeof(data), fp);
//                fclose(fp);
//                printf("created file F:acemor.was.here. it has %d bytes\n", len);
//            }
//            else
//            {
//                printf("failed to create file F:acemor.was.here\n");
//            }
//    
//        }
//        else
...
//    }
    ...
    if(1 == fsal_params_cfgMINE->eram_enable)
    {
        fsal_format(fsal_drive_ram);
        printf("formatted ram drive\n");

        fp = fopen("R:acemor.was.here", "w");
        if(NULL != fp)
        {
            len = fwrite(data, 1, sizeof(data), fp);
            fclose(fp);
            printf("created file R:acemor.was.here. it has %d bytes\n", len);
        }
        else
        {
            printf("failed to create file R:acemor.was.here\n");
        }
