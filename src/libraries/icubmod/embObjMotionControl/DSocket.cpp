...
ACE_SOCK_Dgram_Bcast* DSocket::Create(ACE_UINT16 port,std::string& address)
{
...
ACE_SOCK_Dgram_Bcast* DSocket::Create(ACE_UINT16 port,ACE_UINT32 address)
{
...
ssize_t DSocket::ReceiveFrom(void* data,size_t len)
{
    ACE_INET_Addr ace_addr;
    ssize_t nrec = mSocket->recv(data, len, ace_addr, 0);
//    address=ace_addr.get_host_addr();
//    port=ace_addr.get_port_number();
    return nrec;
}
