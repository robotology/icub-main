...
PERFORMANCE TEST FOR IDYN: time to compute icub's whole body torques

This file is the same as WholeBodyTorqueObserver (revision 7111) 
except for the PERFORMANCE_TEST flag, which enables two YARP ports
transmitting timing measurements.
The flag is hard-coded (#define PERFORMANCE_TEST 1) to ensure minimal
'intrusion' in the original code: a configuration parameter would result
in a sequence of ..if(flag).., introducing a little delay with respect
to the original code.

The created ports are:
1- /wholebody/perf_test/times:o
2- /wholebody/perf_test/ftread:o

sending bottles of this type:

1- [int double double double]
receivedMeasure = 0/1 depending on if a new FT measure has been received (1) or not (0)
startRun        = start time of run()
threadRate      = end of run() - start of run()
computeTime     = end of idyn computations - start of idyn computations, i.e. the time required to 
                  compute the whole body torques by idyn, given the sensor measurements or not

2- [int double double double double]
receivedMeasure = 0/1 depending on if a new FT measure has been received (1) or not (0)
startRun        = start time of run()
processTime     = end of run() – current measure, i.e. the time required to process a received measure, 
                  compute torques and send them to their correct ports, i.e. the time from receiving a 
                  FT measure from a YARP port and send back the torques to a YARP port
...
Authors: Serena Ivaldi (test code), Matteo Fumagalli (author of the original module)
...
#if PERFORMANCE_TEST
    // time instants to be checked for performances
    double startRun;
    double endRun;
    double startCompute;
    double endCompute;
...
public:
    ...
    {        
        ...
        port_perf_test->open("/wholebody/perf_test/times:o");
        port_perf_test_ftRead = new BufferedPort<Bottle>;
        port_perf_test_ftRead->open("/wholebody/perf_test/ftread:o");
#endif
