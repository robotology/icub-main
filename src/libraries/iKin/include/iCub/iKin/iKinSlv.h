 ...
 * \f]
 *
 * Where the solution \f$ \mathbf{q} \f$ is the joints vector with n components
(depending
 * on the task and the current dof configuration) that is guaranteed to be found
within the physical
 * bounds expressed by \f$ \mathbf{q}_L \f$ and \f$ \mathbf{q}_U \f$; \f$
\mathbf{x}_d\equiv\left(x,y,d\right)_d \f$
 * is the positional part of the desired pose, \f$ \mathbf{\alpha}_d \f$ is the
 * desired orientation and \f$ \mathit{K_x} \f$ and \f$ \mathit{K_{\alpha}} \f$
are
 * the forward kinematic maps for the position and orientation part,
respectively;
 * \f$ \mathbf{q}_r \f$ is used to keep the solution as close as possible to a
given rest
 * position in the joint space (weighting with a positive factor \f$ \mathit{w}
< 1 \f$ and also
