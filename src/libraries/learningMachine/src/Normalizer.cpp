...
void Normalizer::writeBottle(yarp::os::Bottle& bot) {
    bot << this->lowerBound << this->upperBound << this->lowest << this->highest;

    // make sure to call the superclass's method
    this->IScaler::writeBottle(bot);
}

void Normalizer::readBottle(yarp::os::Bottle& bot) {
    // make sure to call the superclass's method (will reset transformer)
    this->IScaler::readBottle(bot);

    bot >> this->highest >> this->lowest >> this->upperBound >> this->lowerBound;
}

bool Normalizer::configure(yarp::os::Searchable& config) {
