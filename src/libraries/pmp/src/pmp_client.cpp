...
/************************************************************************/
PmpClient::PmpClient() : isOpen(false), verbosity(0)
{
    remote="";
    local="";
...
/************************************************************************/
int PmpClient::printMessage(const int level, const char *format, ...) const
{
    if (verbosity>=level)
    {
...
/************************************************************************/
bool PmpClient::open(const Property &options)
{
    ...
    {        
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_PING);
...
/************************************************************************/
bool PmpClient::addItem(const Property &options, int &item)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_ADD);
        cmd.add(val);
...
/************************************************************************/
bool PmpClient::eraseItem(const int item)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_DEL);
        cmd.add(item);
...
bool PmpClient::clearItems()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_CLEAR);
...
bool PmpClient::getItems(Bottle &items)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_LIST);
...
/************************************************************************/
bool PmpClient::setProperty(const int item, const Property &options)
{
    if (isOpen)
    {     
        string options_str=options.toString().c_str();
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_SET);
        cmd.addInt(item);
        cmd.add(val);
...
bool PmpClient::getProperty(const int item, Property &options)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_GET);
        cmd.addInt(item);
...
bool PmpClient::enableField()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_ENFIELD);
...
bool PmpClient::disableField()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_DISFIELD);
...
bool PmpClient::getFieldStatus(bool &status)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_STATFIELD);
...
bool PmpClient::enableControl()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_ENCTRL);
...
bool PmpClient::disableControl()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_DISCTRL);
...
bool PmpClient::getControlStatus(bool &status)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_STATCTRL);
...
/************************************************************************/
bool PmpClient::enableSimulation()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_ENSIM);
...
/************************************************************************/
bool PmpClient::disableSimulation()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_DISSIM);
...
bool PmpClient::getSimulationStatus(bool &status)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_STATSIM);
...
/************************************************************************/
bool PmpClient::setPeriod(const int period)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_SETPER);
        cmd.addInt(period);
...
bool PmpClient::getPeriod(int &period)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_GETPER);
...
/************************************************************************/
bool PmpClient::setPointStateToTool()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_SETSTATETOTOOL);
...
/************************************************************************/
bool PmpClient::attachToolFrame(const yarp::sig::Vector &x, const yarp::sig::Vector &o)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_ATTACHTOOLFRAME);
        cmd.add(val);
...
bool PmpClient::getToolFrame(yarp::sig::Vector &x, yarp::sig::Vector &o)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_GETTOOLFRAME);
...
/************************************************************************/
bool PmpClient::removeToolFrame()
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_REMOVETOOLFRAME);
...
bool PmpClient::getTool(yarp::sig::Vector &x, yarp::sig::Vector &o)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_GETTOOL);
...
/************************************************************************/
bool PmpClient::setPointState(const Vector &x, const Vector &o,
                              const Vector &xdot, const Vector &odot)
{
    if (isOpen)
    {
        ...
        Bottle cmd,reply;
        cmd.addVocab(PMP_VOCAB_CMD_SETSTATE);
        cmd.add(val);
