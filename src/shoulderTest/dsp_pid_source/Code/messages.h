...
{ \
	calibrate (axis, BYTE_W(CAN_DATA[1], CAN_DATA[2])); \
...
{ \
	writeToFlash (_flash_addr, _board_ID); \
	_general_board_error = ERROR_NONE; \
}
...
{ \
	...
	{ \
		...
		else */ if (_control_mode[axis] != MODE_IDLE && IS_DONE(axis)) \
		{ \
			_control_mode[axis] = MODE_POSITION; \
			_set_point[axis] = BYTE_C(CAN_DATA[1], CAN_DATA[2], CAN_DATA[3], CAN_DATA[4]); \
...
{ \
	...
	{ \
		...
		_conversion_factor[axis] = (tmp * 3.3) / 360360.0f; \
		_general_board_error = ERROR_NONE; \
	} \
	else \
		_general_board_error = ERROR_FMT; \
