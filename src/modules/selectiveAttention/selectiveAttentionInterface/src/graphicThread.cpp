...
static selectiveAttentionModule *module;
...
void graphicThread::setselectiveAttentionModule(void* moduleReference){
    module=(selectiveAttentionModule*) moduleReference;
}

void graphicThread::setName(yarp::os::ConstString value){
    fname=value;
}
...
void graphicThread::outPorts(){
    ...
    if(strcmp("",command->c_str())){
        ...
        commandBottle.clear();
        commandBottle.addVocab(VOCAB3(command->at(0),command->at(1),command->at(2)));
        commandBottle.addVocab(VOCAB3(command->at(4),command->at(5),command->at(6)));
        if(command->length()>7){
...
static void cb_draw_value( GtkToggleButton *button )
{
    ...
    if(!strcmp(button->button.label_text,"Red1-->")){
        if(button->active){
            //selectiveAttentionModule->processor1->redPlane_flag=1;
            //selectiveAttentionModule->processor1->greenPlane_flag=0;
            //selectiveAttentionModule->processor1->bluePlane_flag=0;
        }
    }
    else if(!strcmp(button->button.label_text,"Green1-->")){
        if(button->active){
            //selectiveAttentionModule->processor1->redPlane_flag=0;
            //selectiveAttentionModule->processor1->greenPlane_flag=1;
            //selectiveAttentionModule->processor1->bluePlane_flag=0;
        }
    }
        ...
        if(button->active){
            //selectiveAttentionModule->processor1->redPlane_flag=0;
            //selectiveAttentionModule->processor1->greenPlane_flag=0;
            //selectiveAttentionModule->processor1->bluePlane_flag=1;
        }
    }
...
static void callback( GtkWidget *widget,gpointer   data ){
    ...
    if(!strcmp((char *)data,"OpencvSobel")){
        printf("OpencvSobel");
        module->command->assign("set ocv");
    }
    if(!strcmp((char *)data,"ConvolveMax")){
        printf("ConvolveMax");
        module->command->assign("set max");
    }
    if(!strcmp((char *)data,"ConvolveSeq")){
        printf("ConvolveSeq");
        module->command->assign("set seq");
    }
    if(!strcmp((char *)data,"ConvolveFil")){
        printf("ConvolvFil");
        module->command->assign("set fil");
    }
    if(!strcmp((char *)data,"IppSobel")){
        printf("IppSobel");
        module->command->assign("set ipp");
    }
...
GtkWidget* graphicThread::createMainWindow(void)
{
    ...
    boxButton = xpm_label_box (NULL,(gpointer)"output1");
    boxButton2= xpm_label_box (NULL,(gpointer)"output2");
    ...
    adj2 = gtk_adjustment_new (0.0, 0.0,1.0,0.01, 0.0, 0.0);
    ...
    adj3 = gtk_adjustment_new (0.0, 0.0,1.0,0.01, 0.0, 0.0);
    ...
    adj4 = gtk_adjustment_new (0.0, 0.0,1.0,0.01, 0.0, 0.0);
    ...
    adj5 = gtk_adjustment_new (0.0, 0.0,1.0,0.01, 0.0, 0.0);
    ...
    adj6 = gtk_adjustment_new (0.0, 0.0,1.0,0.01, 0.0, 0.0);
