...
void IPortEventListener::resetPort(std::string portName) {
    ...
    if(yarp::os::Network::queryName(portName.c_str()).isValid()) {
...
bool IPortEventListener::configure(yarp::os::Searchable& config) {
    bool success = this->IEventListener::configure(config);

    // enable verbose output
    if(!config.findGroup("verbose").isNull()) {
        this->verbose = true;
        success = true;
    }

    // enable silent output
    if(!config.findGroup("silent").isNull()) {
        this->verbose = false;
        success = true;
    }
