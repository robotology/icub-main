...
void IMachineLearnerModule::registerPort(yarp::os::Contactable& port, std::string name) {
...
void IMachineLearnerModule::loadCommandFile(std::string fname, yarp::os::Bottle* out) {
    FileReaderT<yarp::os::Bottle> str;
    yarp::os::Bottle* cmd = (yarp::os::Bottle*) 0;
    ...
    while(str.hasNext()) {
        cmd = str.getNext();
        reply.clear();
        if(*cmd != yarp::os::Bottle::getNullBottle()) {
            if(out != (yarp::os::Bottle*) 0) {
