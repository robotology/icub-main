...
#           tmp.inCheck.grid(row=r, column=3, sticky=W)
    ...
    def runModules(self):
        ...
        for mod in self.application.modules:
            if (mod.stdioNode == ""):
                cmd=['yarp', 'run', '--cmd', '\"'+mod.name+' '+mod.parameters+'\"', '--on', '/'+mod.node, '--as', mod.tag]
            else:
                cmd=['yarp', 'run', '--cmd', '\"'+mod.name+' '+mod.parameters+'\"', '--on', '/'+mod.node, '--as', mod.tag, '--stdio', '/'+mod.stdioNode]
...
if __name__ == '__main__':
    ...
    for app in applications:
        ...
        for c in app.getElementsByTagName("connection"):
            input=(c.getElementsByTagName("input")[0].firstChild.data)
            output=(c.getElementsByTagName("output")[0].firstChild.data)
