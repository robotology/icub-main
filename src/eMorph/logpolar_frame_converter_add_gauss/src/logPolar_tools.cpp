...
void C_logpolarTools::initData()
{
    ...
    int r_tmp_1 = (int)sqrt(float((cart_width-x_0)*(cart_width-x_0)+(cart_height-y_0)*(cart_height-y_0)));
    r_max = r_tmp_1;
...
void C_logpolarTools::createTables()
{
    ...
    for(int i=0; i<cart_width;i++)
        for(int j=0; j<cart_height; j++)
        {
            r=sqrt(float((i-x_0)*(i-x_0)+(j-y_0)*(j-y_0)));
            if(floor(r+0.5)>r_0)
            {
                rho_xy[i][j]=floor(M*log(r+alpha)-beta/*+0.5*/);
                _atan = atan2(float(j-y_0), float(i-x_0));
                if(_atan < 0)
                    _atan = _atan+2*M_PI;
