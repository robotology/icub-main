...
C_converter::C_converter(int i_width, int i_height):lp_tools(i_width,i_height)
{
    ...
    port.open("/image/logpolar_add_gauss");
}
...
//            cout << '\t' << _x << " " << _y << " " << _pol << " " << _ts << endl;
            ...
            if(_x!=-1 && _y!=-1)
            {
                ...
                {
                    acc[_rho][_phi] += _pol;
                    int num_pixels = lp_tools.get_x(_rho, _phi).size();
//                    switch(sign(acc[_rho][_phi]))
//                    {
//                        case -1: tmp_lImg(_rho, _phi)=(int)floor(125.0-(float)acc[_rho][_phi]*600.0*((1/(theta*sqrt(2*3.1415)))*exp(-0.5*((((float)num_pixels-mu)/theta)*(((float)num_pixels-mu)/theta)))));//tmp_lImg(_rho, _phi)>=25?tmp_lImg(_rho, _phi)+_pol*25:0/*0*/;
//                            if(tmp_lImg(_rho, _phi)<0)
//                                tmp_lImg(_rho, _phi) = 0;
//                            break;
//                        case 1: tmp_lImg(_rho, _phi)=(int)floor(125.0+(float)acc[_rho][_phi]*600.0*((1/(theta*sqrt(2*3.1415)))*exp(-0.5*((((float)num_pixels-mu)/theta)*(((float)num_pixels-mu)/theta)))));//tmp_lImg(_rho, _phi)<=225?tmp_lImg(_rho, _phi)+_pol*25:255/*255*/;
//                            if(tmp_lImg(_rho, _phi)>255)
//                                tmp_lImg(_rho, _phi) = 255;
//                            break;
//                        case 0: tmp_lImg(_rho, _phi)=125; break;
//                    }
                    switch(sign(_pol/*acc[_rho][_phi]*/))
                    {
                        case -1: tmp_lImg(_rho, _phi)+=(int)floor((float)_pol*600.0*((1/(theta*sqrt(2*3.1415)))*exp(-0.5*((((float)num_pixels-mu)/theta)*(((float)num_pixels-mu)/theta)))));//tmp_lImg(_rho, _phi)>=25?tmp_lImg(_rho, _phi)+_pol*25:0/*0*/;
                            if(tmp_lImg(_rho, _phi)<0)
                                tmp_lImg(_rho, _phi) = 0;
                            break;
                        case 1: tmp_lImg(_rho, _phi)+=(int)floor((float)_pol*600.0*((1/(theta*sqrt(2*3.1415)))*exp(-0.5*((((float)num_pixels-mu)/theta)*(((float)num_pixels-mu)/theta)))));//tmp_lImg(_rho, _phi)<=225?tmp_lImg(_rho, _phi)+_pol*25:255/*255*/;
                            if(tmp_lImg(_rho, _phi)>255)
                                tmp_lImg(_rho, _phi) = 255;
                            break;
                        case 0: tmp_lImg(_rho, _phi)=125; break;
